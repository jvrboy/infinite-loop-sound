...
const getFileIcon = (extension?: string) => {
  const map: Record<string, { color: string; icon: string }> = {
    tsx: { color: "text-cyan-400", icon: "TS" },
    ts: { color: "text-blue-400", icon: "TS" },
    jsx: { color: "text-cyan-300", icon: "JS" },
    js: { color: "text-yellow-400", icon: "JS" },
    css: { color: "text-violet-400", icon: "CS" },
    json: { color: "text-amber-300", icon: "{}" },
    md: { color: "text-muted-foreground", icon: "MD" },
    svg: { color: "text-emerald-400", icon: "SV" },
    png: { color: "text-emerald-300", icon: "IM" },
    default: { color: "text-muted-foreground", icon: "·" },
  };
  return map[extension || "default"] || map.default;
};
...
function FileItem({ node, depth, isLast, parentPath }: FileItemProps) {
  ...
  return (
    <div className="select-none">
      <div
        className={cn(
          "group relative flex items-center gap-2 py-1 px-2 rounded-md cursor-pointer transition-all duration-200 ease-out",
