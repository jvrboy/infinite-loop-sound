...
import {
  Zap, Filter, Send, Info, ImageIcon, ImageOff, Download, BarChart3, TrendingUp, TrendingDown,
  ChevronDown, ChevronUp, Clock, Target, Search, ArrowUpDown, LayoutGrid, List, X
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import {
  ALL_ASSETS, ASSETS_BY_CLASS, TIMEFRAMES, displayPair, type AssetClass, type TF
...
export const Route = createFileRoute("/signals")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Professional signal management with advanced filtering, live price tracking, and batch operations." },
    ],
  }),
...
type Row = SignalCardData & { id: string; created_at: string; status?: string; result?: string | null; closed_at?: string | null };
...
function SignalsPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("signals").select("*").order("created_at", { ascending: false }).limit(500)
      .then(({ data }) => { setSignals((data as any[]) ?? []); setLoading(false); });
    const ch = supabase.channel("signals-list")
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "signals" },
        (p) => setSignals(prev => prev.map(x => x.id === (p.new as any).id ? { ...x, ...(p.new as any) } : x)))
      .subscribe();
    return () => { supabase.removeChannel(ch); };
  }, []);

  const filtered = useMemo(() => {
    let s = signals.filter(s => {
      ...
      if (searchQuery) {
        ...
        if (!displayPair(s.pair).toLowerCase().includes(q) &&
            !s.direction.toLowerCase().includes(q) &&
            !s.rating.toLowerCase().includes(q)) return false;
      }
      return true;
    });
    s.sort((a, b) => {
    ...
    return s;
  }, [signals, pairs, tfs, dirs, ratings, statuses, reqInd, minScore, searchQuery, sortField, sortDir]);

  const paged = useMemo(() => filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE), [filtered, page]);
  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));

  useEffect(() => { setPage(1); }, [filtered.length, searchQuery]);
  ...
  }, [filtered.slice(0, 30).map(s => s.pair).join(",")]);
  ...
  const broadcastAll = async () => {
    let sent = 0;
    for (const s of filtered.slice(0, 20)) { try { await send(s); sent++; } catch {} }
    toast.success(`Broadcast attempted on ${sent} signals`);
  };

  const clearAll = () => {
    setPairs(new Set()); setTfs(new Set()); setDirs(new Set()); setRatings(new Set());
    setStatuses(new Set(["active", "pending"])); setReqInd(new Set()); setMinScore(0); setSearchQuery("");
  };

  const activeCount = pairs.size + tfs.size + dirs.size + ratings.size + statuses.size + reqInd.size + (minScore > 0 ? 1 : 0) + (searchQuery ? 1 : 0);

  const exportCSV = () => {
    const headers = ["ID", "Pair", "Timeframe", "Direction", "Score", "Rating", "Status", "Result", "Entry", "SL", "TP1", "TP2", "TP3", "Created", "Closed"];
    const rows = filtered.map(s => [
      s.id, s.pair, s.timeframe, s.direction, s.score, s.rating, s.status || "-", s.result || "-",
      s.entry, s.sl, s.tp1, s.tp2, s.tp3,
      new Date(s.created_at).toISOString(), s.closed_at || "-"
    ]);
  ...
  const stats = useMemo(() => {
    ...
    const won = signals.filter(s => (s.result || "").toUpperCase().startsWith("TP") || (s.result || "").toUpperCase() === "WIN").length;
    const lost = signals.filter(s => (s.result || "").toUpperCase() === "SL" || (s.result || "").toUpperCase() === "LOSS").length;
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">{filtered.length} of {signals.length} signals matching filters</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button size="sm" variant="outline" onClick={exportCSV} disabled={!filtered.length}>
              <Download className="w-3.5 h-3.5 mr-1.5" /> Export
            </Button>
            <Button size="sm" variant="outline" onClick={() => update({ telegramChartSnapshots: !settings.telegramChartSnapshots })}>
            ...
            <Button size="sm" variant="outline" onClick={() => setShowFilters(!showFilters)}>
              ...
              Filters{activeCount > 0 && <span className="ml-1.5 px-1.5 py-0.5 rounded bg-elite/20 text-elite text-[10px]">{activeCount}</span>}
            </Button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          ...
          <StatBadge icon={Zap} label="Win Rate" value={`${stats.winRate}%`} color={stats.winRate >= 60 ? "text-bull" : stats.winRate >= 40 ? "text-medium" : "text-bear"} />
        </div>
          ...
          <Card className="border-primary/20">
            <CardContent className="p-4 space-y-3">
              ...
              <FilterGroup label="Status">
                {STATUS_OPTIONS.map(s => <Chip key={s} active={statuses.has(s)} onClick={() => toggleSet(statuses, s, setStatuses)}>{s.toUpperCase()}</Chip>)}
              </FilterGroup>
          ...
          <Card className="overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {paged.map(s => (
                    <tr key={s.id} className="border-t border-border hover:bg-accent/20 transition cursor-pointer" onClick={() => setDrawerSig(s)}>
                      <td className="px-3 py-2">
                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${s.rating === "ELITE" ? "bg-elite/20 text-elite" : s.rating === "STRONG" ? "bg-bull/20 text-bull" : "bg-muted text-muted-foreground"}`}>
                          {s.rating}
                        </span>
                      </td>
                      <td className="px-3 py-2 font-mono font-semibold">{displayPair(s.pair)}</td>
                      <td className="px-3 py-2 text-center font-mono text-xs text-muted-foreground">{s.timeframe}</td>
                      ...
                      <td className="px-3 py-2 text-right font-mono text-bear">{s.sl?.toFixed(5)}</td>
                      <td className="px-3 py-2 text-right font-mono text-bull">{s.tp3?.toFixed(5)}</td>
                      <td className="px-3 py-2">
                        <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded ${
                          !s.result && (!s.status || s.status === "active" || s.status === "pending") ? "bg-bull/10 text-bull" :
                          (s.result || "").toUpperCase().startsWith("TP") || (s.result || "").toUpperCase() === "WIN" ? "bg-bull/20 text-bull" :
                          (s.result || "").toUpperCase() === "SL" || (s.result || "").toUpperCase() === "LOSS" ? "bg-bear/20 text-bear" :
                          "bg-muted text-muted-foreground"
                        }`}>{s.result || s.status || "ACTIVE"}</span>
                      </td>
                      <td className="px-3 py-2 text-right">
                        <button onClick={e => { e.stopPropagation(); send(s); }} className="text-xs text-primary hover:underline">Send</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
...
function StatBadge({ icon: Icon, label, value, color }: { icon: any; label: string; value: string | number; color?: string }) {
  return (
    <div className="rounded-lg border border-border bg-card/60 p-2 flex items-center gap-2">
      ...
      <div>
        <div className={`text-lg font-bold font-mono leading-tight ${color || "text-foreground"}`}>{value}</div>
