...
function runSimulations(opts: {
  ...
  for (let r = 0; r < opts.nRuns; r++) {
    ...
    for (let i = 1; i <= opts.trades; i++) {
      if (bal <= 0) { if (blowupAt === null) blowupAt = i; curve.push(0); continue; }
...
function SimulatorPage() {
  ...
  const [hist, setHist] = useState<HistoricalStats>({
    trades: 0, wins: 0, losses: 0, winRate: 55, source: "default",
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const wins = data.filter((t: any) => t.status === "win" || t.status === "closed_profit").length;
      const losses = data.filter((t: any) => t.status === "loss" || t.status === "closed_loss" || t.status === "error").length;
      const n = wins + losses;
      if (n >= 20) {
  ...
  const { chartData, stats } = useMemo(() => {
    ...
    const runs = runSimulations({
      startingBalance, trades, winRate, riskReward, riskPerTrade, nRuns: N_RUNS,
    });
    // For each trade index, gather all balances across runs and compute percentiles
    const len = trades + 1;
    const cd: Array<{ trade: number; p10: number; p25: number; p50: number; p75: number; p90: number }> = [];
    for (let i = 0; i < len; i++) {
        ...
        <div className="flex justify-between items-end flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              ...
              {hist.source === "live" && (
                <> · prefilled from <code className="px-1 rounded bg-muted/40">bot_trades</code> ({hist.trades} samples)</>
              )}
            </p>
          </div>
          <button
        ...
        <div className="grid md:grid-cols-4 gap-6">
          ...
          <div className="glass-card p-5 rounded-lg space-y-4 h-fit">
            <div>
              <label className="text-sm font-medium mb-1.5 block flex justify-between">
                <span>Win rate</span><span className="font-mono text-muted-foreground">{winRate}%</span>
              </label>
              <input type="range" min={1} max={99} value={winRate} onChange={e => setWinRate(+e.target.value)} className="w-full accent-primary" />
            </div>
            <div>
              <label className="text-sm font-medium mb-1.5 block flex justify-between">
                <span>Risk : Reward</span><span className="font-mono text-muted-foreground">1 : {riskReward.toFixed(1)}</span>
              </label>
              <input type="range" min={0.1} max={5} step={0.1} value={riskReward} onChange={e => setRiskReward(+e.target.value)} className="w-full accent-primary" />
            </div>
            <div>
              <label className="text-sm font-medium mb-1.5 block flex justify-between">
                <span>Trades</span><span className="font-mono text-muted-foreground">{trades}</span>
              </label>
              <input type="range" min={10} max={500} step={10} value={trades} onChange={e => setTrades(+e.target.value)} className="w-full accent-primary" />
            </div>
            <div>
              <label className="text-sm font-medium mb-1.5 block flex justify-between">
                <span>Risk per trade</span><span className="font-mono text-muted-foreground">{riskPerTrade}%</span>
              </label>
              <input type="range" min={0.1} max={10} step={0.1} value={riskPerTrade} onChange={e => setRiskPerTrade(+e.target.value)} className="w-full accent-primary" />
            </div>
            <div>
            ...
            <div className="pt-4 mt-4 border-t border-border space-y-2.5 text-sm">
              <Stat label="Expected edge" v={`${stats.edge >= 0 ? "+" : ""}${stats.edge.toFixed(2)}R`} good={stats.edge > 0} />
              <Stat label="Profit probability" v={`${(stats.profitProb * 100).toFixed(0)}%`} good={stats.profitProb >= 0.5} />
              <Stat label="Median finish" v={`$${Math.round(stats.median).toLocaleString()}`} good={stats.median > startingBalance} />
              <Stat label="10th pct (bad)" v={`$${Math.round(stats.p10).toLocaleString()}`} good={stats.p10 > startingBalance} />
              ...
              <Stat label="Blow-up rate" v={`${(stats.blowupRate * 100).toFixed(1)}%`} good={stats.blowupRate < 0.05} />
            </div>
          </div>
          ...
          <div className="md:col-span-3 glass-card p-5 rounded-lg h-[500px] flex flex-col">
            ...
            <div className="flex-1 min-h-0 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fill: "var(--muted-foreground)", fontSize: 11 }} tickFormatter={v => `$${v}`} width={70} domain={["auto", "auto"]} />
                  <Tooltip
                    contentStyle={{ backgroundColor: "var(--popover)", border: "1px solid var(--border)", borderRadius: 6, fontSize: 12 }}
                    formatter={(v: any, name: any) => [`$${Math.round(Number(v)).toLocaleString()}`, name]}
                  />
                  {/* 10–90 band */}
                  <Area type="monotone" dataKey="p90" stroke="none" fill="url(#band90)" stackId="band90" />
                  <Area type="monotone" dataKey="p10" stroke="none" fill="var(--background)" stackId="band90" />
                  {/* 25–75 band on top */}
                  <Area type="monotone" dataKey="p75" stroke="none" fill="url(#band75)" stackId="band75" />
                  <Area type="monotone" dataKey="p25" stroke="none" fill="var(--background)" stackId="band75" />
                  {/* median line */}
            ...
            <div className="flex flex-wrap items-center gap-3 mt-3 text-[10px] text-muted-foreground font-mono">
              <span className="flex items-center gap-1.5"><span className="w-3 h-2 rounded" style={{ background: "rgba(59,130,246,0.25)" }} /> 10–90 pct</span>
              <span className="flex items-center gap-1.5"><span className="w-3 h-2 rounded" style={{ background: "rgba(59,130,246,0.35)" }} /> 25–75 pct</span>
              <span className="flex items-center gap-1.5"><span className="w-3 h-2 rounded" style={{ background: "#10b981" }} /> Median</span>
              <span className="ml-auto flex items-center gap-1"><History className="w-3 h-3" /> {hist.source === "live" ? `historical: ${hist.wins}W / ${hist.losses}L` : "no historical sample yet"}</span>
            </div>
          </div>
        </div>

        <p className="text-[10px] text-muted-foreground text-center">
          Distribution computed over {N_RUNS} independent Monte Carlo runs. Inputs default to your actual bot_trades win rate when ≥20 trades exist.
        </p>
      </div>
    </AppShell>
  );
