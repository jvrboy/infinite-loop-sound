...
import { Brain, Zap, TrendingUp, TrendingDown, Activity, Target, BarChart3, RefreshCw, Cpu, Gauge, ArrowUpCircle, ArrowDownCircle, AlertTriangle, CheckCircle2, XCircle, Timer, Flame } from "lucide-react";
...
export const Route = createFileRoute("/boom-crash")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Neural network-powered Boom & Crash index predictor with self-improving accuracy" },
    ],
  }),
...
function BoomCrashPage() {
  ...
  const markOutcome = (signal: BoomCrashSignal, hit: boolean) => {
    ...
    setSignals(prev => prev.map(s => s.id === signal.id ? { ...s, outcome: hit ? "HIT" : "MISS", outcomeTimestamp: new Date().toISOString() } : s));
    setNNStats(getNNStats());
    ...
    <AppShell>
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              className="gap-2"
            >
              {autoScan ? <><XCircle className="w-4 h-4" /> Stop Auto</> : <><RefreshCw className="w-4 h-4" /> Auto Scan</>}
            </Button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          {[
            { label: "NN Version", value: `v${nnStats.version}`, icon: Cpu, color: "text-cyan-400" },
            { label: "Training Data", value: nnStats.trainingSize.toString(), icon: Brain, color: "text-violet-400" },
            { label: "Accuracy", value: `${nnStats.accuracy}%`, icon: Target, color: nnStats.accuracy >= 65 ? "text-bull" : "text-bear" },
            { label: "Learning Rate", value: nnStats.learningRate.toFixed(4), icon: Activity, color: "text-amber-400" },
            { label: "Win Rate", value: stats.total ? `${((stats.hits / Math.max(1, stats.hits + stats.misses)) * 100).toFixed(0)}%` : "—", icon: CheckCircle2, color: "text-emerald-400" },
            { label: "Signals", value: stats.total.toString(), icon: BarChart3, color: "text-blue-400" },
          ].map(s => (
        ...
        <div className="glass-card rounded-xl p-4">
          <div className="text-xs uppercase tracking-wider text-muted-foreground mb-3 font-semibold">Select Index</div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
            {BOOM_CRASH_INDICES.map(idx => (
              ...
              >
                ...
                <div className="text-[10px] text-muted-foreground">{idx.type === "boom" ? "Spike Up" : "Spike Down"}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        {tickHistory.length > 0 && (
          <div className="glass-card rounded-xl p-4">
            <div className="text-xs uppercase tracking-wider text-muted-foreground mb-3 font-semibold flex items-center gap-2">
              ...
              {autoScan && <span className="inline-flex items-center gap-1 text-bull"><span className="w-2 h-2 rounded-full bg-bull animate-pulse" /> Live</span>}
            </div>
                ...
                return (
                  <div
                    ...
                    className={`flex-1 min-w-[1px] rounded-t-sm transition-all ${
                      isSpike ? (tick > prev ? "bg-bull shadow-glow-bull" : "bg-bear shadow-glow-bear") : "bg-primary/30"
                    }`}
                    style={{ height: `${height}%` }}
                  />
                );
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 text-xs">
              <span className="text-muted-foreground">
                <CheckCircle2 className="w-3 h-3 inline mr-1 text-bull" />{stats.hits} hits
              </span>
              <span className="text-muted-foreground">
                <XCircle className="w-3 h-3 inline mr-1 text-bear" />{stats.misses} misses
              </span>
              <span className="text-muted-foreground">
                <Timer className="w-3 h-3 inline mr-1" />{stats.pending} pending
              </span>
            </div>
          </div>
          ...
          {signals.length === 0 ? (
            <div className="glass-card rounded-xl p-12 text-center">
              ...
              <p className="text-muted-foreground">No predictions yet. Select an index and click Scan Now.</p>
              <p className="text-xs text-muted-foreground/70 mt-1">The neural network improves with every outcome you mark.</p>
            </div>
          ) : (
            signals.map(signal => (
              <div key={signal.id} className="glass-card glass-card-hover rounded-xl overflow-hidden">
                <div className="p-4 md:p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl grid place-items-center ${
                        signal.direction === "SPIKE_UP" ? "bg-bull/20 text-bull" : "bg-bear/20 text-bear"
                      }`}>
                        {signal.direction === "SPIKE_UP" ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" />}
                      </div>
                      <div>
                        <div className="font-bold">{signal.index}</div>
                        <div className="text-xs text-muted-foreground flex items-center gap-2">
                          <span className={signal.direction === "SPIKE_UP" ? "text-bull" : "text-bear"}>
                    ...
                    <div className="flex items-center gap-2">
                      {signal.outcome ? (
                        <span className={`px-2.5 py-1 rounded-lg text-xs font-bold ${
                          signal.outcome === "HIT" ? "bg-bull/20 text-bull" : "bg-bear/20 text-bear"
                        }`}>
                          {signal.outcome}
                        </span>
                      ) : (
                        <>
                          <Button size="sm" variant="ghost" className="h-7 px-2 text-bull hover:bg-bull/10" onClick={() => markOutcome(signal, true)}>
                            <CheckCircle2 className="w-3.5 h-3.5 mr-1" /> Hit
                          </Button>
                          <Button size="sm" variant="ghost" className="h-7 px-2 text-bear hover:bg-bear/10" onClick={() => markOutcome(signal, false)}>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-3">
                    <div className="text-center">
                      <div className={`text-2xl font-bold font-mono ${signal.neuralScore >= 70 ? "text-bull" : signal.neuralScore >= 50 ? "text-elite" : "text-bear"}`}>
                        {signal.neuralScore}
                      </div>
                      <div className="text-[10px] text-muted-foreground">Neural Score</div>
                    </div>
                    <div className="text-center">
                      <div className={`text-2xl font-bold font-mono ${signal.confidence >= 70 ? "text-bull" : "text-elite"}`}>
                        {signal.confidence}%
                      </div>
                    ...
                    {signal.factors.map(f => (
                      <div key={f.name} className="text-xs">
                        <div className="flex justify-between mb-0.5">
                          ...
                          <span className={f.signal === "bullish" ? "text-bull" : f.signal === "bearish" ? "text-bear" : "text-muted-foreground"}>
                        ...
                        <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all ${
                              f.signal === "bullish" ? "bg-bull" : f.signal === "bearish" ? "bg-bear" : "bg-muted-foreground/50"
                            }`}
