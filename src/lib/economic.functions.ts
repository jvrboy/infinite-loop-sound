...
export const refreshEconomicEvents = createServerFn({ method: "POST" })
  .handler(async () => {
    const r = await fetch(SOURCE, { headers: { Accept: "application/json", "User-Agent": "DivergenceIQ/1.0" } });
...
export const listEconomicEvents = createServerFn({ method: "GET" })
  .handler(async () => {
    const sb = admin();
    const { data, error } = await sb.from("economic_events_cache")
      .select("*").gte("event_time", new Date(Date.now() - 86400000).toISOString())
      .order("event_time", { ascending: true }).limit(100);
    if (error) throw new Error(error.message);
    return { events: data || [] };
  });