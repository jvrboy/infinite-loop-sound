...
export const saveSignal = createServerFn({ method: "POST" })
  .validator((d) => z.object({
    ...
    score: z.number().int(), rating: z.string(),
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      await Promise.allSettled((hooks || []).filter((h: any) => (row.score ?? 0) >= (h.min_score ?? 0)).map(async (h: any) => {
        ...
        await sb.from("webhook_subscriptions").update({ last_delivery_at: new Date().toISOString() }).eq("id", h.id);
      }));
    } catch { /* ignore */ }
