...
export function createDefaultChannel(name: string, steps: number): ChannelRackChannel {
  return {
    id: crypto.randomUUID(), name, color: "#3b82f6", sampleId: null,
...
export function createDefaultChannelRack(): ChannelRackState {
  const steps = 16;
  return {
    id: crypto.randomUUID(), name: "Pattern 1", bpm: 120, steps, stepsPerBeat: 4,
    channels: [
      createDefaultChannel("Kick", steps), createDefaultChannel("Snare", steps),
      createDefaultChannel("Hi-Hat", steps), createDefaultChannel("Clap", steps),
      createDefaultChannel("Perc", steps), createDefaultChannel("Bass", steps),
      createDefaultChannel("Synth", steps), createDefaultChannel("FX", steps),
    ],
...
export function updateStep(channel: ChannelRackChannel, stepIndex: number, patch: Partial<ChannelRackStep>): ChannelRackChannel {
  const steps = [...channel.steps];
...
export function updateChannel(rack: ChannelRackState, channelId: string, patch: Partial<ChannelRackChannel>): ChannelRackState {
  return { ...rack, channels: rack.channels.map((c) => (c.id === channelId ? { ...c, ...patch } : c)) };
}

export function assignSample(rack: ChannelRackState, channelId: string, sample: Sample): ChannelRackState {
  return { ...rack, channels: rack.channels.map((c) => (c.id === channelId ? { ...c, sampleId: sample.id, sample, name: c.name || sample.name } : c)) };
}
...
export function resizeSteps(rack: ChannelRackState, newSteps: number): ChannelRackState {
  return {
    ...
    channels: rack.channels.map((c) => {
      const steps = [...c.steps];
      if (newSteps > steps.length) { while (steps.length < newSteps) steps.push(createDefaultStep()); }
      else { steps.length = newSteps; }
      return { ...c, steps };
    }),
...
export function playStep(ctx: AudioContext, channel: ChannelRackChannel, step: ChannelRackStep, startTime: number): void {
  ...
  for (let r = 0; r < retriggers; r++) {
    ...
    playOneShot(ctx, channel.sample, { gain, pitch: step.pitch, pan: step.pan + channel.pan, startTime: t });
  }
}
...
export class ChannelRackScheduler {
  ...
  constructor(ctx: AudioContext, rack: ChannelRackState) { this.ctx = ctx; this.rack = rack; }
  update(rack: ChannelRackState) { this.rack = rack; }
  start() {
  ...
  private schedule = () => {
    ...
    while (this.nextStepTime < this.ctx.currentTime + 0.1) {
      ...
      if (this.rack.loopMode && this.currentStep === this.rack.loopEnd + 1) this.currentStep = this.rack.loopStart;
    }
