...
export function detectRegime(candles: Candle[]): {
  ...
  if (candles.length < 20) {
    return { regime: "quiet", adx: 0, volPct: 0, drift: 0, confidence: 0, approach: "Insufficient data." };
  }
  ...
  const ret = closes.slice(-20).map((v, i, arr) => (i === 0 ? 0 : (v - arr[i - 1]) / (arr[i - 1] || 1)));
...
export function liquidityMap(candles: Candle[]): {
  ...
  const buyStr = clusters.filter((x) => x.side.startsWith("buy")).reduce((s, x) => s + x.strength, 0);
  const sellStr = clusters.filter((x) => x.side.startsWith("sell")).reduce((s, x) => s + x.strength, 0);
...
export function replayTicks(ticks: Tick[], speed = 1): {
  vwap: number;
  buyPressure: number;
  sellPressure: number;
  deltaCum: number;
  ...
  let pv = 0, vol = 0, buy = 0, sell = 0, delta = 0;
  ...
  for (let i = 0; i < ticks.length; i++) {
    ...
    vol += tk.volume;
    if (side === "buy") { buy += tk.volume; delta += tk.volume; }
    else { sell += tk.volume; delta -= tk.volume; }
...
export function orderFlowImbalance(candles: Candle[]): {
  ...
  const absorption = !!last && last.v > avgVol * 1.8 && Math.abs(last.c - last.o) < (last.h - last.l) * 0.3;
