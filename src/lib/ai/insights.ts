...
function generateRecommendation(trade: any, allTrades: any[]): string {
  const similarTrades = allTrades.filter(t => t.pair === trade.pair && t.direction === trade.direction);
  const similarWinRate = similarTrades.length > 0 
    ? (similarTrades.filter(t => t.result === 'WIN').length / similarTrades.length) * 100 
    : 50;
  
  if (trade.result === 'WIN' && similarWinRate > 60) return 'Repeat this setup, excellent edge detected';
  if (trade.result === 'LOSS' && similarWinRate < 30) return 'Avoid this pair/direction combination';
...
function calculateAnomalyScore(trade: any, allTrades: any[]): number {
  ...
  const stdDev = Math.sqrt(allTrades.reduce((sum, t) => sum + Math.pow(t.pnl - avgPnl, 2), 0) / allTrades.length);
