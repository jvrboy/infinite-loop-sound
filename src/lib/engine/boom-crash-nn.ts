...
export class BoomCrashNeuralNetwork {
  ...
  // Forward pass
  predict(features: number[]): { spikeProbability: number; directionBias: number; confidence: number; hiddenActivations: number[] } {
  ...
  train(features: number[], actualOutcome: number): { loss: number; improvement: number } {
    ...
    const outputGradients = this.weightsHO.map((w, i) => {
      const o = this.sigmoid(w.reduce((acc, wt, j) => acc + wt * prediction.hiddenActivations[j], 0) + this.biasO[i]);
      const target = i === 0 ? actualOutcome : (i === 1 ? (actualOutcome > 0.5 ? 0.8 : 0.2) : Math.abs(error) < 0.2 ? 0.9 : 0.3);
    ...
    for (let i = 0; i < this.outputSize; i++) {
      for (let j = 0; j < this.hiddenSize; j++) {
        this.weightsHO[i][j] += this.learningRate * outputGradients[i] * prediction.hiddenActivations[j];
      }
    ...
    const recentAccuracy = recent.filter(t => Math.abs(t.actual - Math.round(t.predicted)) < 0.5).length / recentN;
  ...
  saveWeights() {
    try {
      localStorage.setItem("diq_bcnn_weights", JSON.stringify({
...
export function extractBoomCrashFeatures(ticks: number[], index: string): number[] {
  ...
  const recentSpikes = recent.slice(-20).filter((v, i) => i > 0 && Math.abs(v - recent[n - 20 + i - 1]) > spikeThreshold).length;
  ...
  const vol50 = Math.sqrt(returns.slice(-50).reduce((a, b) => a + b ** 2, 0) / Math.min(50, returns.length));
  ...
  // Skewness
  const skew = returns.reduce((a, r) => a + ((r - avgReturn) / (returnStd || 0.001)) ** 3, 0) / returns.length;
  const normSkew = Math.min(1, Math.max(0, (skew + 3) / 6));

  // Kurtosis (heavy tails = more spikes)
  const kurt = returns.reduce((a, r) => a + ((r - avgReturn) / (returnStd || 0.001)) ** 4, 0) / returns.length - 3;
...
export function generateBoomCrashSignal(ticks: number[], index: string): BoomCrashSignal | null {
  ...
  const isBoom = index.toLowerCase().includes("boom");
  const direction: "SPIKE_UP" | "SPIKE_DOWN" = prediction.directionBias > 0.5 ? "SPIKE_UP" : "SPIKE_DOWN";
  ...
  const factors: BoomCrashFactor[] = [
    { name: "Spike Probability", value: prediction.spikeProbability, weight: 0.3, signal: prediction.spikeProbability > 0.65 ? "bullish" : "neutral" },
    { name: "Volatility Regime", value: features[5], weight: 0.15, signal: features[5] > 0.6 ? "bullish" : features[5] < 0.3 ? "bearish" : "neutral" },
    { name: "Compression", value: features[9], weight: 0.2, signal: features[9] > 0.5 ? "bullish" : "neutral" },
    { name: "RSI", value: features[3], weight: 0.1, signal: features[3] > 0.7 ? "bearish" : features[3] < 0.3 ? "bullish" : "neutral" },
    { name: "Momentum", value: features[2], weight: 0.1, signal: features[2] > 0.6 ? "bullish" : features[2] < 0.4 ? "bearish" : "neutral" },
    { name: "Tick Gap", value: features[4], weight: 0.15, signal: features[4] > 0.7 ? "bullish" : "neutral" },
  ];
  ...
  return {
    ...
    confidence,
    predictedTick: Math.round(currentTick * (1 + (isBoom ? 0.005 : -0.005) * prediction.spikeProbability)),
    currentTick,
