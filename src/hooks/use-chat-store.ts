...
export function useThreads() {
  ...
  useEffect(() => { setThreads(safeRead<Thread[]>(THREADS_KEY, [])); }, []);

  const persist = useCallback((t: Thread[]) => { setThreads(t); safeWrite(THREADS_KEY, t); }, []);

  const create = useCallback((title = "New chat", folderId: string | null = null): Thread => {
    const t: Thread = { id: crypto.randomUUID(), title, messages: [], updated: Date.now(), folderId };
    persist([t, ...threads]);
    return t;
  }, [persist, threads]);
  ...
  const remove = useCallback((id: string) => {
    persist(threads.map((t) => (t.id === id ? { ...t, deleted: true, deletedAt: Date.now(), pinned: false } : t)));
  }, [persist, threads]);
  ...
  const restore = useCallback((id: string) => {
    persist(threads.map((t) => (t.id === id ? { ...t, deleted: false, deletedAt: undefined } : t)));
  }, [persist, threads]);
  ...
  const clone = useCallback((id: string): Thread | null => {
    const original = threads.find((t) => t.id === id);
    if (!original) return null;
    const copy: Thread = {
      ...original, id: crypto.randomUUID(),
      title: `${original.title} (clone)`, updated: Date.now(),
      pinned: false, archived: false, deleted: false, deletedAt: undefined, clonedFrom: id,
    };
    persist([copy, ...threads]);
    return copy;
  }, [persist, threads]);

  const move = useCallback((id: string, folderId: string | null) => {
  ...
  return { threads, persist, create, update, remove, hardRemove, restore, emptyTrash, clone, move, togglePin, toggleArchive };
}

export function useFolders() {
  const [folders, setFolders] = useState<ChatFolder[]>([]);
  useEffect(() => { setFolders(safeRead<ChatFolder[]>(FOLDERS_KEY, [])); }, []);

  const persist = useCallback((f: ChatFolder[]) => { setFolders(f); safeWrite(FOLDERS_KEY, f); }, []);

  const create = useCallback((name: string, color = "#3b82f6"): ChatFolder => {
    const f: ChatFolder = { id: crypto.randomUUID(), name, color, order: folders.length, createdAt: Date.now() };
    persist([...folders, f]);
    return f;
  }, [persist, folders]);
  ...
  const reorder = useCallback((id: string, newOrder: number) => {
...
export function useArtifacts(threadId?: string | null) {
  ...
  useEffect(() => { setAll(safeRead<Artifact[]>(ARTIFACTS_KEY, [])); }, []);

  const persist = useCallback((a: Artifact[]) => { setAll(a); safeWrite(ARTIFACTS_KEY, a); }, []);

  const add = useCallback((a: Omit<Artifact, "id" | "createdAt">) => {
    const full: Artifact = { ...a, id: crypto.randomUUID(), createdAt: Date.now() };
    persist([full, ...all]);
    return full;
  }, [persist, all]);

  const remove = useCallback((id: string) => persist(all.filter((a) => a.id !== id)), [persist, all]);
...
export function useUsage() {
  ...
  useEffect(() => { setEvents(safeRead<UsageEvent[]>(USAGE_KEY, [])); }, []);
