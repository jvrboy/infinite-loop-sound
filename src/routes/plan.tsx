...
const DEFAULT_PLAN = [
  ...
  { id: 3, text: "Are there at least 3 indicators showing confluence?", checked: false, critical: false },
    ...
    <AppShell>
      <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Run through your custom rules before entering any position.</p>
        </div>
        ...
        <div className="bg-card border border-border p-6 rounded-lg space-y-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4 flex-1">
              <div className="w-16 h-16 rounded-full border-4 border-muted flex items-center justify-center relative">
                <svg className="absolute inset-0 w-full h-full -rotate-90">
                  <circle cx="30" cy="30" r="28" fill="none" stroke="currentColor" strokeWidth="4" className="text-muted opacity-20" />
                  <circle cx="30" cy="30" r="28" fill="none" stroke="currentColor" strokeWidth="4" 
                    ...
                    className={progress === 100 ? "text-bull transition-all duration-500" : "text-primary transition-all duration-500"} 
                  />
                </svg>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {progress === 100 ? "All systems go. You are cleared to trade." : "Complete your checklist before trading."}
                </p>
            ...
            <button onClick={resetChecks} className="px-3 py-1.5 text-xs border border-border rounded hover:bg-accent transition">
              Reset Checks
            </button>
          </div>
          ...
          <div className="space-y-2">
            {items.map(item => (
              <div key={item.id} className={`flex items-center gap-3 p-3 rounded-lg border transition ${item.checked ? "bg-primary/5 border-primary/20" : "bg-background border-border hover:border-primary/50"}`}>
                <button 
                ...
                >
                  {item.checked && <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                </button>
                <span className={`flex-1 text-sm ${item.checked ? "text-muted-foreground line-through" : ""}`}>{item.text}</span>
                {item.critical && <span className="text-[9px] uppercase tracking-wider bg-bear/10 text-bear px-1.5 py-0.5 rounded">Critical</span>}
                <button onClick={() => remove(item.id)} className="text-muted-foreground hover:text-bear opacity-50 hover:opacity-100 transition p-1">
