...
function JournalPage() {
  ...
  const addEntry = async () => {
    ...
    const tags = tagsBuf.split(",").map(s => s.trim()).filter(Boolean);
    ...
    setNote(""); setPair(""); setOutcome(null); setRr(""); setTagsBuf("");
  };
  ...
  const remove = async (id: string) => {
    ...
    if (authed) {
      ...
      setEntries(next); writeLocal(next);
    }
  };
      ...
      <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { l: "Win Rate",  v: `${stats.winRate.toFixed(0)}%`,           s: `${stats.wins}W ${stats.loss}L ${stats.be}BE` },
            { l: "Net R",     v: stats.rTotal.toFixed(2),                  s: "sum of RR"           },
            { l: "Outcomes",  v: `${stats.wins+stats.loss+stats.be}/${stats.total}`, s: "graded vs total" },
          ].map(s => (
        ...
        <div className="glass-card rounded-lg p-4 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-[120px_1fr_120px_120px] gap-2">
            <Input placeholder="Pair" value={pair} onChange={e => setPair(e.target.value)} className="font-mono text-xs" />
            <Input placeholder="Note (Cmd/Ctrl-Enter to save)" value={note}
              onChange={e => setNote(e.target.value)}
              onKeyDown={e => { if ((e.metaKey || e.ctrlKey) && e.key === "Enter") addEntry(); }}
              className="text-sm" />
            <Input placeholder="R (e.g. 2.5)" value={rr} onChange={e => setRr(e.target.value)} className="font-mono text-xs" />
            <Button onClick={addEntry} className="gap-1.5"><Plus className="w-3.5 h-3.5" /> Add</Button>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {OUTCOMES.map(o => (
              <button key={o.id} onClick={() => setOutcome(outcome === o.id ? null : o.id)}
                className={`text-[10px] px-2 py-1 rounded font-mono uppercase tracking-wider diq-press ${
                  outcome === o.id ? o.color + " ring-1 ring-current" : "bg-muted/40 text-muted-foreground"
                }`}>
                {o.label}
              </button>
            ))}
        ...
        <div className="glass-card rounded-lg p-3 flex flex-wrap items-center gap-2">
          ...
          {(["ALL", ...OUTCOMES.map(o => o.id)] as const).map(o => (
            <button key={o} onClick={() => setFilterOutcome(o as Outcome | "ALL")}
              className={`text-[10px] px-2 py-1 rounded font-mono uppercase tracking-wider diq-press ${
                filterOutcome === o ? "bg-primary text-primary-foreground" : "bg-muted/40 text-muted-foreground"
              }`}>
              {o}
            </button>
          ))}
        ...
        <div className="grid gap-3">
          {loading && entries.length === 0 && (
            <div className="text-center text-muted-foreground text-sm py-8 italic">Loading entries…</div>
          )}
                ...
                {isEditing ? (
                  <div className="space-y-2">
                    <div className="grid grid-cols-1 md:grid-cols-[120px_1fr_120px] gap-2">
                      <Input value={editDraft.pair as string ?? ""}     onChange={ev => setEditDraft({ ...editDraft, pair: ev.target.value })} className="font-mono text-xs" />
                      <Input value={editDraft.note as string ?? ""}     onChange={ev => setEditDraft({ ...editDraft, note: ev.target.value })} />
                      <Input value={editDraft.rr?.toString() ?? ""}      onChange={ev => setEditDraft({ ...editDraft, rr: ev.target.value ? Number(ev.target.value) : null })} className="font-mono text-xs" placeholder="R" />
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      {OUTCOMES.map(o => (
                        <button key={o.id} onClick={() => setEditDraft({ ...editDraft, outcome: editDraft.outcome === o.id ? null : o.id })}
                          className={`text-[10px] px-2 py-1 rounded font-mono uppercase tracking-wider ${
                            editDraft.outcome === o.id ? o.color + " ring-1 ring-current" : "bg-muted/40 text-muted-foreground"
                          }`}>
                          {o.label}
                        </button>
                      ))}
                      <Input
                        ...
                        onChange={ev => setEditDraft({ ...editDraft, tags: ev.target.value.split(",").map(s => s.trim()).filter(Boolean) })}
                        className="flex-1 min-w-[160px] text-xs"
                      />
                      <Button size="sm" onClick={saveEdit} className="gap-1"><Save className="w-3.5 h-3.5" /> Save</Button>
                      <Button size="sm" variant="ghost" onClick={() => { setEditingId(null); setEditDraft({}); }}><X className="w-3.5 h-3.5" /></Button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-3 flex-wrap">
                        <span className="font-mono font-bold text-sm bg-muted/60 px-2 py-0.5 rounded">{e.pair}</span>
                        <span className="text-xs text-muted-foreground">{new Date(e.created_at).toLocaleString("en-ZA", { timeZone: "Africa/Johannesburg", hour12: false })}</span>
                        {oc && (
                          <span className={`text-[10px] px-2 py-0.5 rounded uppercase tracking-wider font-mono ${oc.color}`}>{oc.label}</span>
                        )}
                        {typeof e.rr === "number" && (
                          <span className={`text-[10px] font-mono px-2 py-0.5 rounded ${e.rr >= 0 ? "bg-bull/15 text-bull" : "bg-bear/15 text-bear"}`}>
                            {e.rr >= 0 ? "+" : ""}{e.rr.toFixed(2)}R
                          </span>
                        )}
                      </div>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                        <button onClick={() => startEdit(e)} className="text-muted-foreground hover:text-primary p-1"><Pencil className="w-3.5 h-3.5" /></button>
                        <button onClick={() => remove(e.id)} className="text-muted-foreground hover:text-red-400 p-1"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </div>
                    <p className="text-sm text-foreground/90 leading-relaxed whitespace-pre-wrap">{e.note}</p>
                    {e.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {e.tags.map(t => (
                          <span key={t} className="text-[10px] uppercase tracking-wider bg-primary/10 text-primary px-2 py-0.5 rounded-full inline-flex items-center gap-1">
