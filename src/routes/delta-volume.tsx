...
function calcDelta(candles: Candle[]) {
  ...
  for (let i = 1; i < deltas.length; i++) {
    ...
    if ((priceUp && deltaDown) || (priceDown && deltaUp)) {
      divergence.push({ index: i, type: priceUp ? "BEARISH DIV" : "BULLISH DIV", price: candles[i].c, cumDelta: deltas[i].cumDelta });
    }
  }

  return { deltas, totalDelta, posDelta, negDelta, deltaRatio, lastPrice, lastDelta, lastCumDelta, divergence };
}

function DeltaVolumePage() {
  const [input, setInput] = useState("1.080,1.083,5000\n1.083,1.081,7000\n1.081,1.085,9000\n1.085,1.082,6000\n1.082,1.087,11000\n1.087,1.084,8000\n1.084,1.089,13000\n1.089,1.086,5000\n1.086,1.091,15000\n1.091,1.088,7000\n1.088,1.093,12000\n1.093,1.090,4000\n1.090,1.085,9000\n1.085,1.082,11000\n1.082,1.088,8000");
              ...
              <>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-card border border-border p-4 rounded-lg text-center">
                    ...
                    <div className={`font-mono font-bold text-2xl mt-1 ${result.totalDelta >= 0 ? "text-bull" : "text-bear"}`}>
                      {result.totalDelta >= 0 ? "+" : ""}{fmt(result.totalDelta)}
                    </div>
                  </div>
                  <div className="bg-card border border-border p-4 rounded-lg text-center">
                    <div className="text-[10px] text-muted-foreground uppercase">Delta Ratio</div>
                    <div className="font-mono font-bold text-2xl mt-1">{result.deltaRatio.toFixed(2)}</div>
                  </div>
                </div>
                    ...
                    {result.deltas.map((d, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 flex items-center gap-1">
                          <div className={`flex-1 h-3 rounded ${d.isUp ? "bg-bull/60" : "bg-bear/60"}`} style={{ width: `${Math.min(100, (Math.abs(d.delta) / result.posDelta) * 100)}%`, minWidth: "4px" }} />
                        </div>
                        <span className={`text-[10px] font-mono w-12 text-right ${d.delta >= 0 ? "text-bull" : "text-bear"}`}>{d.delta >= 0 ? "+" : ""}{fmt(d.delta)}</span>
                        <span className="text-[10px] font-mono w-16 text-right text-muted-foreground">{fmt(d.cumDelta)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    <div className="space-y-1">
                      {result.divergence.map((d, i) => (
                        <div key={i} className="flex justify-between text-xs">
                          <span className={`font-bold ${d.type.includes("BULL") ? "text-bull" : "text-bear"}`}>{d.type}</span>
                          <span className="font-mono text-muted-foreground">Bar {d.index} · Price {d.price.toFixed(5)} · CumΔ {fmt(d.cumDelta)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
