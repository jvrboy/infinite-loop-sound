...
function calcVwap(candles: Candle[], deviations = [1, 2, 3]) {
  ...
  for (const b of bands) {
    if (lastClose > b.upper) { band = `ABOVE +${b.dev}σ`; break; }
    if (lastClose < b.lower) { band = `BELOW -${b.dev}σ`; break; }
  }
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Volume-Weighted Average Price with standard deviation bands for institutional fair-value detection.
          </p>
        </div>
            ...
            {result && (
              <>
                <div className="bg-card border border-border p-6 rounded-lg text-center">
                  ...
                  <div className={`text-sm mt-2 font-medium ${result.aboveVwap ? "text-bull" : "text-bear"}`}>
                    {result.aboveVwap ? "+" : ""}{result.distPct.toFixed(3)}% from VWAP
                  </div>
                  <div className="mt-2"><span className="px-3 py-1 bg-muted rounded-full text-xs font-bold">{result.band}</span></div>
                </div>

                <div className="bg-card border border-border rounded-lg overflow-hidden">
                  <div className="bg-muted/50 p-3 border-b border-border font-semibold">Standard Deviation Bands</div>
