...
import { Mic, Play, Square, Upload, Download, Music, Sliders, RefreshCw, Activity } from "lucide-react";
import {
  recordVoice, synthesizeVoice, generateVariations, parseLyrics, applyVariation,
  type VoiceSample, type VoiceSynthOptions, type LyricSyllable,
...
export const Route = createFileRoute("/voice-studio")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Singing voice synthesis: record your voice, type lyrics, and generate vocal performances with variations." },
    ],
  }),
...
function VoiceStudioPage() {
  ...
  const [opts, setOpts] = useState<VoiceSynthOptions>({
    voiceId: "", lyrics: "", melody: [], tempo: 120,
    variation: "all", randomness: 0.5, formantShift: 0, breathiness: 0.1,
  });
  ...
  const handleLoadVoice = async (file: File) => {
    ...
    setVoice({
      id: crypto.randomUUID(), name: file.name, buffer: audioBuf, phonemes,
      pitchRange: { min: 48, max: 72 }, formants: { f1: 700, f2: 1220, f3: 2600, pitch: 150 },
    });
  };
  ...
  const handleSynthesize = async () => {
    ...
    setBusy(true);
    try { setResult(await synthesizeVoice(ctxRef.current, voice, opts)); }
  ...
  const handleVariations = async () => {
    if (!voice || !ctxRef.current) return;
    setBusy(true);
    try { setVariations(await generateVariations(ctxRef.current, voice, opts, 4)); }
    catch (e) { console.error(e); } finally { setBusy(false); }
  };
  ...
  return (
    <AppShell>
      <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6">
        ...
        <ProCard title="Voice Sample" description="Record from microphone or load an existing voice sample." icon={<Mic className="w-4 h-4" />}>
          <div className="flex gap-2 items-center flex-wrap">
            <Button onClick={handleRecord} disabled={recording}>
              {recording ? `Recording… ${Math.round(recordProgress * 100)}%` : "Record Voice (5s)"}
            </Button>
            <Input type="file" accept="audio/*" onChange={(e) => e.target.files?.[0] && handleLoadVoice(e.target.files[0])} className="flex-1" />
          </div>
        </ProCard>

        <ProCard title="Lyrics & Melody" description="Type lyrics and set the melody. Syllables are auto-parsed." icon={<Music className="w-4 h-4" />}>
          <div className="space-y-3">
            <div>
              <Label>Lyrics</Label>
              <textarea className="w-full bg-background border border-border rounded px-3 py-2 text-sm min-h-[80px]"
                value={lyrics} onChange={(e) => setLyrics(e.target.value)} placeholder="Type your lyrics here…" />
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <Label>Tempo {opts.tempo} BPM</Label>
                <input type="range" min={60} max={200} value={opts.tempo}
                  onChange={(e) => setOpts((p) => ({ ...p, tempo: Number(e.target.value) }))} className="w-full" />
              </div>
              <div>
                <Label>Variation</Label>
                <select className="w-full bg-background border border-border rounded px-2 py-2 text-sm" value={opts.variation}
                  onChange={(e) => setOpts((p) => ({ ...p, variation: e.target.value as VoiceSynthOptions["variation"] }))}>
              ...
              <div>
                ...
                <input type="range" min={0} max={1} step={0.05} value={opts.randomness}
                  onChange={(e) => setOpts((p) => ({ ...p, randomness: Number(e.target.value) }))} className="w-full" />
              </div>
              <div>
                <Label>Formant Shift {opts.formantShift}</Label>
                <input type="range" min={-12} max={12} value={opts.formantShift}
                  onChange={(e) => setOpts((p) => ({ ...p, formantShift: Number(e.target.value) }))} className="w-full" />
              </div>
              <div>
                <Label>Breathiness {Math.round(opts.breathiness * 100)}%</Label>
                <input type="range" min={0} max={1} step={0.05} value={opts.breathiness}
                  onChange={(e) => setOpts((p) => ({ ...p, breathiness: Number(e.target.value) }))} className="w-full" />
              </div>
            </div>
        ...
        {melody.length > 0 && (
          <ProCard title="Parsed Syllables" description={`${melody.length} syllables detected from lyrics.`} icon={<Activity className="w-4 h-4" />}>
        ...
        {result && (
          <ProCard title="Result" description="Synthesized vocal performance." icon={<Play className="w-4 h-4" />}>
            <div className="flex gap-2">
        ...
        {variations.length > 0 && (
          <ProCard title="Variations" description={`${variations.length} generated variations.`} icon={<RefreshCw className="w-4 h-4" />}>
              ...
              {variations.map((v, i) => (
                <div key={i} className="p-2 rounded border border-border bg-card">
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button size="sm" variant="outline" onClick={() => handleDownload(v, `variation-${i + 1}.wav`)}>
