...
export const getAutomationStateFn = createServerFn({ method: "GET" })
  .handler(async () => {
    const sb = admin();
    ...
    return {
      ...
      stats: {
        ...
        bySchedule: (schedules ?? []).reduce<Record<string, { signals: number; dispatched: number }>>((acc, s: any) => {
...
export const getAutomationSignalsFn = createServerFn({ method: "GET" })
  .handler(async () => {
    const sb = admin();
...
export const loadPresetSchedulesFn = createServerFn({ method: "POST" })
  .handler(async () => {
    const sb = admin();
