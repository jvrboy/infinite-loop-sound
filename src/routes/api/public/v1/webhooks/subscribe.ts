  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        const secret = p.data.secret ?? crypto.randomUUID().replace(/-/g, "") + crypto.randomUUID().replace(/-/g, "");
        const { data, error } = await admin().from("webhook_subscriptions").insert({
          url: p.data.url, min_score: p.data.min_score ?? 70, secret,
        }).select().single();
        ...
        return new Response(JSON.stringify({
          id: data.id, url: data.url, min_score: data.min_score, secret,
          verification: "HMAC-SHA256 of body using `secret`; sent as `X-DIQ-Signature: sha256=<hex>` with `X-DIQ-Timestamp`",
