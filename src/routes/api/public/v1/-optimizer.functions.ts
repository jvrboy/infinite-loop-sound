...
export const getOptimizerStateFn = createServerFn({ method: "GET" })
  .handler(async () => {
    const sb = admin();
    ...
    // Per-pair stats
    const pairStats: Record<string, { total: number; wins: number; slHits: number; avgPnl: number; topCause: string }> = {};
    for (const o of all) {
  ...
  .handler(async ({ data }) => {
    ...
    if (data.outcome === "SL_HIT") {
      ...
      if (slOvershoot >= 0 && slOvershoot < slDist * 0.3 && favorable > slDist * 0.8) {
        rootCauses.push({
          category: "sl_placement", severity: "critical",
      ...
      misleadingConfluence = data.confluence
        .filter((c) => c.passed)
        .map((c) => c.label);
      ...
      if (passedCount < 3) {
        rootCauses.push({
          category: "insufficient_confluence", severity: "major",
