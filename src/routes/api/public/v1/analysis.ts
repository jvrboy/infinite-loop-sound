...
const TF_GRAN: Record<string, number> = { M1:60, M5:300, M15:900, M30:1800, H1:3600, H4:14400, D1:86400 };
...
async function fetchCandles(symbol: string, gran: number, count = 250) {
  ...
  return await new Promise<any[]>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => { try { ws.close(); } catch {} reject(new Error("Deriv timeout")); }, 12000);
    ws.addEventListener("open", () => {
      ws.send(JSON.stringify({ ticks_history: symbol, adjust_start_time: 1, count, end: "latest", granularity: gran, style: "candles" }));
    });
    ...
    ws.addEventListener("error", () => { clearTimeout(timeout); reject(new Error("Deriv WS error")); });
  });
}
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (!p.success) return new Response(JSON.stringify({ error: p.error.message }), { status: 400, headers: { "content-type": "application/json", ...CORS } });
        try {
          ...
          return new Response(JSON.stringify({
            pair: r.pair, timeframe: r.timeframe, direction: r.direction,
            score: r.score, scorePct: r.scorePct, rating: r.rating, trade: r.trade,
            confluence: r.confluence,
            divergences: r.divergences.map(d => ({ name: d.name, detected: !!d.result?.type, type: d.result?.type ?? null })),
          }), { headers: { "content-type": "application/json", ...CORS } });
        } catch (e: any) {
          return new Response(JSON.stringify({ error: e.message || "Analysis failed" }), { status: 500, headers: { "content-type": "application/json", ...CORS } });
        }
      },
    },
  },
