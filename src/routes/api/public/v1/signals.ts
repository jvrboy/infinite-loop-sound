  ...
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        let q = admin().from("signals").select("*").order("created_at", { ascending: false }).limit(limit).gte("score", minScore);
        ...
        return new Response(JSON.stringify({ count: data?.length ?? 0, signals: data ?? [] }), { headers: { "content-type": "application/json", ...CORS } });
      },
    },
  },
});