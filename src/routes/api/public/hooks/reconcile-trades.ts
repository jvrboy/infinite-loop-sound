  ...
  server: {
    handlers: {
      POST: async () => {
        ...
        const { data: open } = await sb.from("bot_trades").select("*")
          .in("status", ["open", "pending"]).not("contract_id", "is", null).limit(50);
        ...
        for (const t of open || []) {
          try {
            ...
            if (!tokenFallback) { results.push({ id: t.id, skipped: "no-token" }); continue; }
            const c = await fetchContract(tokenFallback, String(t.contract_id));
            if (!c?.is_settled) { results.push({ id: t.id, pending: true }); continue; }
            const profit = Number(c.profit ?? 0);
            const payout = Number(c.payout ?? 0);
            const status = profit > 0 ? "won" : profit < 0 ? "lost" : "expired";
            await sb.from("bot_trades").update({
              status, profit, payout, closed_at: new Date().toISOString(),
            }).eq("id", t.id);
            results.push({ id: t.id, status, profit });

            // Upsert daily PnL
            const day = new Date().toISOString().slice(0, 10);
            const { data: existing } = await sb.from("bot_pnl_daily").select("*").eq("day", day).maybeSingle();
            if (existing) {
              await sb.from("bot_pnl_daily").update({
            ...
            await sb.from("bot_trades").update({ retry_count: retry, last_error_at: new Date().toISOString(), error: e.message }).eq("id", t.id);
            await sb.from("bot_trades_dlq").insert({
              trade_id: t.id, contract_id: String(t.contract_id ?? ""),
              retry_count: retry, last_error: e.message, next_retry_at: nextRetry,
            });
          }
        }
