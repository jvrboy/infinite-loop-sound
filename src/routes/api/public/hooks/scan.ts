...
import { DERIV_WS_URL, TF_TO_GRAN, FOREX_PAIRS, METALS, CRYPTO, SYNTHETICS, INDICES, type TF } from "@/lib/engine/deriv";
...
async function runScan(request: Request): Promise<Response> {
  ...
  const tfs: TF[] = (qp.get("tfs")?.split(",").map(s => s.trim().toUpperCase()).filter(t => VALID_TFS.includes(t as TF)) as TF[]) || DEFAULT_TFS;
  const classes = (qp.get("classes")?.split(",").map(s => s.trim().toLowerCase())) || DEFAULT_CLASSES;
  ...
  const pools: Record<string, Array<{ symbol: string }>> = {
    forex: FOREX_PAIRS, metals: METALS, crypto: CRYPTO,
    synthetics: SYNTHETICS, indices: INDICES,
  };
  ...
  return Response.json({
    ok: true, scanned, saved, hits: hits.length, expired,
