  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!token) {
          await recordWebhookEvent({ source: "telegram", endpoint, ip, signatureValid, statusCode: 500, payload: {}, headers, error: "TELEGRAM_BOT_TOKEN not configured" });
        ...
        if (!signatureValid) {
          await recordWebhookEvent({ source: "telegram", endpoint, ip, signatureValid: false, statusCode: 401, payload: update, headers, error: "Telegram secret token mismatch" });
          return new Response("unauthorized", { status: 401 });
        }

        const msg = update.message ?? update.edited_message;
        if (!msg?.chat?.id) {
          await recordWebhookEvent({ source: "telegram", endpoint, ip, signatureValid, statusCode: 200, payload: update, headers });
        ...
        if (text.startsWith("/start")) {
          ...
          const { data } = await sb.from("signals").select("*")
            .gte("score", minScore).order("created_at", { ascending: false }).limit(5);
          if (!data || data.length === 0) {
            await tg(token, "sendMessage", { chat_id: chatId, text: "No signals yet. Run a scan from the dashboard." });
          } else {
        ...
        await recordWebhookEvent({ source: "telegram", endpoint, ip, signatureValid, statusCode: 200, payload: { chat_id: chatId, text }, headers });
        return Response.json({ ok: true });
      },
    },
  },
