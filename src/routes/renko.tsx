...
function RenkoPage() {
  ...
  const trend = upBricks > downBricks * 1.5 ? "BULLISH" : downBricks > upBricks * 1.5 ? "BEARISH" : "NEUTRAL";
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="bg-card border border-border p-4 rounded-lg space-y-3">
              ...
              <div>
                <label className="text-sm font-medium mb-1 block">Brick Size (0 = auto ATR: {autoBrickSize.toFixed(5)})</label>
                <input type="number" step="0.0001" value={brickSize || ""} onChange={(e) => setBrickSize(Number(e.target.value))} className="w-full p-2 border border-input rounded bg-background font-mono text-sm" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="bg-card border border-border p-3 rounded-lg text-center">
                ...
                <div className={`font-bold text-sm mt-1 ${trend === "BULLISH" ? "text-bull" : trend === "BEARISH" ? "text-bear" : "text-muted-foreground"}`}>{trend}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-lg p-4">
            ...
            <div className="space-y-0.5 max-h-96 overflow-y-auto">
              {bricks.map((b, i) => (
                <div key={i} className={`flex items-center gap-2 p-1.5 rounded ${b.direction === "up" ? "bg-bull/10" : "bg-bear/10"}`}>
                  <div className={`w-6 h-6 rounded grid place-items-center text-xs font-bold ${b.direction === "up" ? "bg-bull/20 text-bull" : "bg-bear/20 text-bear"}`}>
                    {b.direction === "up" ? "▲" : "▼"}
                  </div>
              ...
              {bricks.length === 0 && <div className="text-center text-muted-foreground py-4 text-sm">No bricks generated. Adjust brick size or add more data.</div>}
            </div>
          </div>
        </div>
      </div>
