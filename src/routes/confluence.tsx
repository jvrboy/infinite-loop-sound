...
import {
  Layers, Zap, Brain, Target, TrendingUp, TrendingDown,
  Activity, Loader2, ChevronDown, BarChart3, Shield, Sparkles,
  ArrowUpCircle, ArrowDownCircle, Circle, RefreshCw
} from "lucide-react";
...
export const Route = createFileRoute("/confluence")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Run all 24 strategies across V1/V2/V3 and see real-time confluence analysis." },
    ],
  }),
...
const V1_LABELS = [
  "RSI Divergence", "MACD Divergence", "Stochastic Divergence",
  "RVI Divergence", "OBV Divergence", "EMA 50/200 Aligned",
];

// V2: Momentum + volatility + pattern (8 strategies)
const V2_LABELS = [
  "RVI Line Cross", "RSI Extreme Zone", "Volume / OBV Confirm",
  "ADX Trending (>22)", "ADX Strong (>35)", "BB Squeeze Breakout",
  "Candle Pattern Confirm", "Supertrend Aligned",
];

// V3: Extended confluence + meta strategies (10 strategies)
const V3_LABELS = [
  "Parabolic SAR Aligned", "Ichimoku T/K Aligned", "Williams %R Extreme",
  "CCI Confirms", "MFI Extreme", "EMA 8/21 Momentum",
  "HTF Bias Aligned", "Squeeze Detected (3-bar)", "High Compression (>60%)",
...
const META_CONFLUENCE = [
  ...
  { label: "Ichimoku Cloud Structure", description: "Kumo twist, Chikou span, Senkou span alignment" },
];

function categorize(items: ConfluenceItem[]) {
  const match = (labels: string[]) =>
    items.filter((c) => labels.includes(c.label));
...
function ConfluencePage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      v1, v2, v3, passedV1, passedV2, passedV3,
      agreementScore, buyCount, sellCount,
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {result && (
                <div className="flex items-center gap-2 ml-auto text-xs text-muted-foreground">
                  ...
                  <span className="font-mono">{displayPair(pair)} · {tf}</span>
        ...
        {!result && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-lg font-semibold text-muted-foreground">Select a pair and click Analyze</h3>
              ...
              <Card className="border-border bg-card">
                ...
                <CardContent className="flex flex-col items-center py-4">
                  <div className="relative w-40 h-40">
                    <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
                      ...
                      <circle
                        ...
                        strokeLinecap="round"
                        className={
                          stats.agreementScore >= 60 ? "text-bull" :
                          stats.agreementScore >= 40 ? "text-amber-400" :
                          "text-bear"
                        }
                      />
                    </svg>
                  ...
                  <div className="mt-3 text-center">
                    <div className={`text-lg font-semibold flex items-center gap-1.5 justify-center ${
                      result.direction === "BUY" ? "text-bull" : result.direction === "SELL" ? "text-bear" : "text-muted-foreground"
                    }`}>
                      {result.direction === "BUY" ? <TrendingUp className="w-4 h-4" /> :
                       result.direction === "SELL" ? <TrendingDown className="w-4 h-4" /> : null}
                      {result.direction ?? "NEUTRAL"}
                    </div>
                    <Badge variant={result.rating === "ELITE" ? "default" : result.rating === "STRONG" ? "secondary" : "outline"} className="mt-1">
                      {result.rating}
                    </Badge>
                  </div>
                </CardContent>
                  ...
                  <CardContent>
                    <div className="flex items-center gap-6 mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="text-xl font-bold font-mono text-bull">{stats.buyCount}</div>
                          <div className="text-[10px] text-muted-foreground uppercase">BUY strategies</div>
                        </div>
                      </div>
                      <div className="flex-1 h-3 rounded-full bg-bear/20 overflow-hidden flex">
                        <div
                          ...
                          style={{ width: `${stats.buyCount > 0 ? (stats.buyCount / stats.totalItems) * 100 : 0}%` }}
                        />
                        <div
                          className="h-full bg-bear/70 rounded-r-full transition-all"
                          style={{ width: `${stats.sellCount > 0 ? (stats.sellCount / stats.totalItems) * 100 : 0}%` }}
                        />
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="text-right">
                          <div className="text-xl font-bold font-mono text-bear">{stats.sellCount}</div>
                          <div className="text-[10px] text-muted-foreground uppercase">SELL strategies</div>
                        </div>
                        <ArrowDownCircle className="w-5 h-5 text-bear" />
                      </div>
                    </div>
                    <div className="text-xs text-muted-foreground text-center">
                      {stats.totalPassed} of {stats.totalItems} strategies triggered ({stats.agreementScore}% agreement)
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="border-border bg-card">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        { label: "V1 Core", passed: stats.passedV1, total: stats.v1.length, color: "text-sky-400", bg: "bg-sky-500/10" },
                        { label: "V2 Momentum", passed: stats.passedV2, total: stats.v2.length, color: "text-violet-400", bg: "bg-violet-500/10" },
                        { label: "V3 Extended", passed: stats.passedV3, total: stats.v3.length, color: "text-emerald-400", bg: "bg-emerald-500/10" },
                      ].map((v) => (
                        <div key={v.label} className={`p-3 rounded-lg ${v.bg} text-center`}>
                          <div className="text-[10px] uppercase text-muted-foreground mb-1">{v.label}</div>
                          <div className={`text-2xl font-bold font-mono ${v.color}`}>
                            {v.passed}<span className="text-sm text-muted-foreground">/{v.total}</span>
                          </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <Card className="border-border bg-card">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>Multi-session, harmonic, SMC, and Ichimoku structure checks</CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card className="border-border bg-card">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>AI-powered score boost based on pattern recognition</CardDescription>
                </CardHeader>
                <CardContent>
                  {neuralBoost ? (
                    <div className="space-y-4">
                      <div className="flex items-center justify-between p-4 rounded-lg bg-gradient-to-br from-pink-500/10 to-violet-500/10 border border-pink-500/20">
                        <div>
                          <div className="text-[10px] uppercase text-muted-foreground">Neural Score</div>
                          <div className="text-3xl font-bold font-mono text-pink-400">{neuralBoost.score}</div>
                        </div>
                        <div className="text-right">
                          <div className="text-[10px] uppercase text-muted-foreground">Confidence</div>
                          <div className="text-2xl font-bold font-mono text-violet-400">{neuralBoost.confidence}%</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-mono text-pink-400 font-semibold">{neuralBoost.score}%</span>
                      ...
                      {neuralBoost.factors.length > 0 && (
                        <div className="space-y-1.5">
                          <div className="text-[10px] uppercase text-muted-foreground">Activation Factors</div>
                      ...
                      {neuralBoost.direction && (
                        <div className={`flex items-center justify-center gap-2 p-3 rounded-lg text-sm font-semibold ${
...
function StrategyGroupCard({
  title, subtitle, items, icon, color,
}: {
  title: string;
  subtitle: string;
    ...
    <Card className="border-border bg-card">
      ...
      <CardContent>
        <div className="space-y-1.5 max-h-64 overflow-y-auto">
          ...
          {items.length === 0 && (
            <p className="text-xs text-muted-foreground italic py-2 text-center">No strategies in this group</p>
          )}
        </div>
      </CardContent>
    </Card>
