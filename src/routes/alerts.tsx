...
import { Bell, Check, AlertTriangle, AlertCircle, RotateCw, Search, Plus, Trash2, Settings2 } from "lucide-react";
...
function AlertsPage() {
  ...
  const [draft, setDraft] = useState({ name: "", metric: "accuracy_score", comparator: "gte", threshold: 75, direction: "" });

  const refresh = async () => {
    let q = (supabase.from("admin_alerts") as any).select("*").order("created_at", { ascending: false }).limit(200);
    if (!showAck) q = q.eq("acknowledged", false);
    const { data } = await q;
    setAlerts(data || []);
  };
  useEffect(() => { refresh(); }, [showAck]);
  const loadRules = async () => {
    const { data } = await (supabase.from("alert_rules") as any).select("*").order("created_at", { ascending: false });
    setRules(data || []);
  };
  useEffect(() => { loadRules(); }, []);
  ...
  const addRule = async () => {
    ...
    await (supabase.from("alert_rules") as any).insert({ ...draft, direction: draft.direction || null });
    setDraft({ name: "", metric: "accuracy_score", comparator: "gte", threshold: 75, direction: "" });
    loadRules();
  };
  ...
  const filtered = useMemo(() => {
    ...
    return alerts.filter(a =>
      ...
      JSON.stringify(a.context || {}).toLowerCase().includes(s));
  }, [alerts, q]);
  ...
  const icon = (sev: string) => sev === "critical" || sev === "error"
  ...
  const explainMatches = (ctx: any) => {
    ...
    return rules.filter(r => r.enabled).map(r => {
      ...
      if (!Number.isFinite(v)) return { rule: r, value: null, pass: false, reason: "metric missing" };
      const pass = r.comparator === "gte" ? v >= r.threshold
                 : r.comparator === "lte" ? v <= r.threshold
    ...
    <AppShell>
      <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-4">
        <div className="flex flex-wrap items-center gap-2 justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" onClick={replayDLQ} disabled={replaying}>
              <RotateCw className={`w-3.5 h-3.5 mr-1 ${replaying ? "animate-spin" : ""}`} /> Replay DLQ
            </Button>
        ...
        <div className="rounded-lg border border-border bg-card divide-y divide-border">
          {filtered.length === 0 ? (
            <div className="px-4 py-10 text-center text-sm text-muted-foreground">All clear — no active alerts.</div>
          ) : filtered.map((a) => {
            ...
            <div key={a.id} className="p-3 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {a.context && Object.keys(a.context).length > 0 && (
                  <pre className="mt-1 text-[10px] font-mono bg-muted/30 p-2 rounded overflow-x-auto">{JSON.stringify(a.context, null, 2)}</pre>
                )}
                {matches.length > 0 && (
                  <div className="mt-2 space-y-0.5">
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Rule evaluation</div>
                    {matches.map((m, i) => (
                      <div key={i} className="flex items-center gap-2 text-[11px] font-mono">
                        <span className={`px-1.5 py-0.5 rounded ${m.pass ? "bg-bull/20 text-bull" : "bg-muted text-muted-foreground"}`}>
                          {m.pass ? "PASS" : "FAIL"}
                        </span>
              ...
              {!a.acknowledged && <Button size="sm" variant="ghost" onClick={() => ack(a.id)}><Check className="w-3.5 h-3.5" /></Button>}
            </div>
            );
          })}
        </div>
        ...
        <div className="rounded-lg border border-border bg-card">
          <button onClick={() => setShowRules(v => !v)} className="w-full flex items-center justify-between px-4 py-3 text-sm font-semibold">
            <span className="flex items-center gap-2"><Settings2 className="w-4 h-4" /> Alert Rules ({rules.length})</span>
          ...
          {showRules && (
            <div className="border-t border-border p-3 space-y-3">
              <div className="grid md:grid-cols-6 gap-2 items-end">
                ...
                <select value={draft.comparator} onChange={e => setDraft({ ...draft, comparator: e.target.value })}
                  className="bg-input border border-border rounded px-2 py-1.5 text-xs">
                ...
                <input type="number" step="0.01" value={draft.threshold}
                  ...
                  className="bg-input border border-border rounded px-2 py-1.5 text-xs" />
                <Button size="sm" onClick={addRule}><Plus className="w-3.5 h-3.5 mr-1" /> Add</Button>
              </div>
              ...
              <div className="divide-y divide-border">
                {rules.map(r => (
                  <div key={r.id} className="flex items-center gap-2 py-2 text-xs">
                    ...
                    <span className="text-muted-foreground font-mono">{r.metric} {r.comparator === "gte" ? "≥" : r.comparator === "lte" ? "≤" : "="} {r.threshold}</span>
                    {r.direction && <span className="text-medium font-mono">[{r.direction}]</span>}
                    <button onClick={() => deleteRule(r.id)} className="text-muted-foreground hover:text-bear"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                ))}
                {!rules.length && <div className="py-4 text-center text-muted-foreground text-xs">No rules yet. Add one above to trigger custom alerts.</div>}
              </div>
            </div>
          )}
        </div>
