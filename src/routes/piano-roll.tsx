...
import { PianoRoll as PianoIcon, Play, Square, Plus, Copy, Trash2, Scissors, Volume2, Music, Sliders, Zap, Grid3x3, ArrowUpDown } from "lucide-react";
import {
...
export const Route = createFileRoute("/piano-roll")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Advanced piano roll with per-note pitch, length, velocity, pan, and ultra-advanced infinite slide system." },
    ],
  }),
...
function PianoRollPage() {
  ...
  const pixelsPerTick = state.pixelsPerTick * state.zoom;
  ...
  useEffect(() => {
    ...
    for (let midi = 0; midi < 128; midi++) {
      ...
      if (state.showScaleGuides && inScale) {
    ...
    for (let tick = 0; tick < state.totalTicks; tick += state.ticksPerBeat) {
      const x = keyWidth + tick * pixelsPerTick;
      const isBar = tick % ticksPerBar === 0;
      ctx.strokeStyle = isBar ? "#444" : "#222";
      ctx.beginPath();
      ctx.moveTo(x, 0);
    ...
    for (const note of state.notes) {
      ...
      // Velocity indicator (height fill)
  ...
  }, [state, playing, totalWidth, totalHeight, pixelsPerTick, pixelsPerSemitone]);
  ...
  useEffect(() => {
      ...
      for (const note of state.notes) {
        ...
        if (playhead >= noteStart && playhead < noteEnd && !activeNotes.has(note.id)) {
          ...
          AudioEngine.noteOn(`proll-${note.id}`, freq, {
            waveform: "sawtooth", attack: 0.01, decay: 0.1, sustain: 0.8, release: 0.2, detune: 0, gain: (note.velocity / 127) * note.gain * 0.4,
          });
    ...
    animFrameRef.current = requestAnimationFrame(animate);
    return () => { if (animFrameRef.current) cancelAnimationFrame(animFrameRef.current); activeNotes.forEach(id => AudioEngine.noteOff(`proll-${id}`)); };
  ...
  const handleCanvasMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const midi = 127 - Math.floor(y / pixelsPerSemitone);
  ...
  }, [state.notes, state.snapMode, pixelsPerTick, pixelsPerSemitone]);
  ...
  const handleCanvasMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (dragMode === "move" && state.selectedNoteIds.length > 0) {
      ...
      if (note) {
        const newDur = Math.max(SNAP_VALUES[state.snapMode], Math.round(((x - keyWidth) / pixelsPerTick - note.startTick) / SNAP_VALUES[state.snapMode]) * SNAP_VALUES[state.snapMode]);
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "Delete" || e.key === "Backspace") {
        ...
        if (clipboard.length > 0) {
          setState(s => pasteNotes(s, clipboard, 480));
        }
  ...
  }, [state.selectedNoteIds, state.notes, clipboard]);
    ...
    <AppShell>
      <div className="p-4 md:p-6 max-w-[1400px] mx-auto space-y-4">
        <SectionHeader
          ...
          subtitle="Ultra-advanced per-note slide system with infinite glide, per-note velocity/pan/gain, and full editing tools."
        ...
        <ProCard>
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <select value={state.key} onChange={(e) => setState(s => ({ ...s, key: e.target.value }))} className="bg-card border border-border rounded px-2 py-1 text-sm">
                {NOTE_NAMES.map(n => <option key={n} value={n}>{n}</option>)}
              </select>
            </div>
            <div className="flex items-center gap-2">
              <Label className="text-xs">Scale</Label>
              <select value={state.scale} onChange={(e) => setState(s => ({ ...s, scale: e.target.value }))} className="bg-card border border-border rounded px-2 py-1 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <select value={state.snapMode} onChange={(e) => setState(s => ({ ...s, snapMode: e.target.value as SnapMode }))} className="bg-card border border-border rounded px-2 py-1 text-sm">
                {['off','1/1','1/2','1/4','1/8','1/16','1/32','1/64','1/128'].map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div className="flex items-center gap-2">
              <Label className="text-xs">Bars</Label>
              <Input type="number" min={1} max={256} value={state.totalBars}
                onChange={(e) => { const v = parseInt(e.target.value); setState(s => ({ ...s, totalBars: v, totalTicks: v * s.ticksPerBeat * s.beatsPerBar })); }}
                className="w-16 font-mono text-sm" />
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setState(s => selectAll(s))} className="gap-1"><Grid3x3 className="w-3 h-3" /> Select All</Button>
              <Button variant="outline" size="sm" onClick={() => setState(s => deselectAll(s))}>Deselect</Button>
              <Button variant="outline" size="sm" onClick={() => { if (state.selectedNoteIds.length > 0) setClipboard(copyNotes(state, state.selectedNoteIds)); }} className="gap-1"><Copy className="w-3 h-3" /> Copy</Button>
              <Button variant="outline" size="sm" onClick={() => { if (clipboard.length > 0) setState(s => pasteNotes(s, clipboard, 480)); }}>Paste</Button>
              <Button variant="outline" size="sm" onClick={() => { if (state.selectedNoteIds.length > 0) setState(s => duplicateNotes(s, s.selectedNoteIds)); }} className="gap-1"><Plus className="w-3 h-3" /> Dup</Button>
              <Button variant="outline" size="sm" onClick={() => { if (state.selectedNoteIds.length > 0) setState(s => removeNotes(s, s.selectedNoteIds)); }} className="gap-1"><Trash2 className="w-3 h-3" /> Del</Button>
              <Button variant="outline" size="sm" onClick={() => { if (state.selectedNoteIds.length > 0) setState(s => quantizeNotes(s, s.selectedNoteIds, s.snapMode)); }} className="gap-1"><Zap className="w-3 h-3" /> Quantize</Button>
            </div>
          </div>
        </ProCard>
          ...
          <ProCard title={`Note: ${midiToNoteName(selectedNote.midi)}`} description="Per-note properties and ultra-advanced slide system" icon={<Sliders className="w-4 h-4" />}>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
              ...
              <div className="flex items-end gap-2">
                ...
                <Button variant={selectedNote.locked ? "secondary" : "outline"} size="sm" onClick={() => setState(s => updateNote(s, selectedNote.id, { locked: !selectedNote.locked }))}>Lock</Button>
              </div>
            </div>
