  ...
  head: () => ({
    meta: [
      { title: "Local AI — Run GGUF models in your browser | DivergenceIQ" },
      {
        name: "description",
        content:
          "Load any .gguf model and run inference locally with WebAssembly. No server, no API key. Your prompts never leave your device.",
      },
    ],
  }),
  component: LocalAI,
...
function LocalAI() {
  ...
  const loadFrom = useCallback(
    async (source: string | File, retry = 0) => {
      setStatus("loading");
      setError(null);
      setProgress(0);
      setRetryCount(retry);
      ...
      try {
        ...
        if (typeof effectiveSource === "string") {
          try {
            ...
            if (size > MAX_MODEL_SIZE_MB * 1024 * 1024) {
              throw new Error(
                `Model too large (${Math.round(size / 1024 / 1024)}MB). Max ${MAX_MODEL_SIZE_MB}MB for browser.`,
              );
            }
          } catch (e: any) {
            if (e.message?.includes("too large") || e.message?.includes("HuggingFace returned"))
              throw e;
            // HEAD might fail on some CDNs, continue anyway
          }
        } else if (source instanceof File && source.size > MAX_MODEL_SIZE_MB * 1024 * 1024) {
          throw new Error(
            `File too large (${Math.round(source.size / 1024 / 1024)}MB). Max ${MAX_MODEL_SIZE_MB}MB.`,
          );
        }
        ...
        if (typeof effectiveSource === "string") {
          setModelName(
            (typeof source === "string" ? source : "model.gguf").split("/").pop() || "model.gguf",
          );
        ...
        if (retry < MAX_RETRIES && !nonRetryable) {
          ...
          setError(
            `${msg} — retrying in ${delay / 1000}s (attempt ${retry + 1}/${MAX_RETRIES})...`,
          );
    ...
    <AppShell>
      <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
        <header className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4 sm:flex sm:items-end sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Load any .gguf model and run inference on-device with WebAssembly. Prompts never leave
              your browser.
            </p>
          </div>
          <span
            className={`shrink-0 text-[10px] uppercase tracking-wider font-mono px-2 py-1 rounded border ${
              status === "ready"
        ...
        <p className="text-[11px] text-muted-foreground text-center">
          Powered by wllama. Models cached in browser storage (IndexedDB). First load can take
