...
function LocalAI() {
  const engineRef = useRef<{ generate: (p: string, n?: number) => Promise<string>; unload: () => Promise<void>; engine: string } | null>(null);
      ...
      <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
        <header className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4 sm:flex sm:items-end sm:justify-between">
          ...
          <div className="flex flex-col items-end gap-1">
            <Badge variant="outline" className="text-[10px]">{PLATFORM_LABELS[platform]}</Badge>
            <span className={`text-[10px] uppercase tracking-wider font-mono px-2 py-1 rounded border ${
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid sm:grid-cols-3 gap-2">
              {SUGGESTED.map((m) => (
                <button key={m.url} onClick={() => setUrl(m.url)}
            ...
            <div className="space-y-2">
              ...
              {!HF_PROXY && <p className="text-[10px] text-amber-400/80 font-mono">VITE_SUPABASE_URL not set — HF requests go direct and may 401.</p>}
            </div>

            <div className="space-y-2">
              <Label className="text-xs">Or upload a local .gguf file</Label>
              <Input type="file" accept=".gguf" onChange={(e) => { const f = e.target.files?.[0]; if (f) loadFrom(f); }} className="font-mono text-xs" />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {status === "ready" && <Button variant="outline" onClick={unload}>Unload</Button>}
              {modelName && <span className="text-xs font-mono text-muted-foreground truncate">{modelName}</span>}
              {activeEngine && <Badge variant="secondary" className="text-[10px]">Engine: {activeEngine}</Badge>}
            </div>
        ...
        <p className="text-[11px] text-muted-foreground text-center">
          Platform: {PLATFORM_LABELS[platform]}. Models cached locally. First load can take minutes on slow networks.
        </p>
      </div>
    </AppShell>
  );
