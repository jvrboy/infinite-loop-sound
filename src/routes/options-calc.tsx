...
function OptionsCalcPage() {
  ...
  const d1 = (Math.log(S / K) + (rateDec + volDec * volDec / 2) * timeInYears) / (volDec * Math.sqrt(timeInYears));
    ...
    <AppShell>
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Calculate theoretical option premiums and Greeks based on market parameters.</p>
        </div>
          ...
          <div className="bg-card border border-border p-6 rounded-lg space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" value={S} onChange={e => setS(Number(e.target.value))} className="w-full p-2 border border-input rounded bg-background font-mono" />
              </div>
              <div>
                <label className="text-sm font-medium mb-1 block">Strike Price ($)</label>
                <input type="number" value={K} onChange={e => setK(Number(e.target.value))} className="w-full p-2 border border-input rounded bg-background font-mono" />
              </div>
            </div>
            ...
            <div>
              ...
              <input type="range" min="1" max="365" value={T} onChange={e => setT(Number(e.target.value))} className="w-full" />
            </div>
            ...
            <div>
              ...
              <input type="range" min="1" max="150" value={v} onChange={e => setV(Number(e.target.value))} className="w-full" />
            </div>

            <div>
              <label className="text-sm font-medium mb-1 block">Risk-Free Rate (%)</label>
              <input type="number" step="0.1" value={r} onChange={e => setR(Number(e.target.value))} className="w-full p-2 border border-input rounded bg-background font-mono" />
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-card border border-border rounded-lg overflow-hidden flex flex-col text-center">
                <div className="bg-bull/10 p-2 text-xs font-semibold text-bull uppercase tracking-wider border-b border-border">Call Value</div>
                <div className="p-6 text-4xl font-bold font-mono">
                  ${isFinite(callPrice) ? callPrice.toFixed(2) : "0.00"}
                </div>
              </div>
              <div className="bg-card border border-border rounded-lg overflow-hidden flex flex-col text-center">
                <div className="bg-bear/10 p-2 text-xs font-semibold text-bear uppercase tracking-wider border-b border-border">Put Value</div>
                <div className="p-6 text-4xl font-bold font-mono">
            ...
            <div className="bg-card border border-border rounded-lg p-5">
              ...
              <div className="space-y-3 font-mono text-sm">
                <div className="flex justify-between items-center pb-2 border-b border-border">
                  ...
                  <span className="font-bold">{isFinite(deltaCall) ? deltaCall.toFixed(4) : "—"}</span>
                </div>
                <div className="flex justify-between items-center pb-2 border-b border-border">
                  <span className="text-muted-foreground">Put Delta (Δ)</span>
                  <span className="font-bold">{isFinite(deltaPut) ? deltaPut.toFixed(4) : "—"}</span>
                </div>
                <div className="flex justify-between items-center pt-1 text-xs text-muted-foreground">
                  <span>Moneyness</span>
                  <span>{S > K ? "In The Money (Call)" : S < K ? "Out of The Money (Call)" : "At The Money"}</span>
                </div>
              </div>
            </div>
          </div>
