...
export const Route = createFileRoute("/audio-tools")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Professional audio analysis: BPM detection, key detection, LUFS metering, spectrum analysis, stem splitting, and more." },
    ],
  }),
...
function AudioToolsPage() {
  ...
  const [pitch, setPitch] = useState<{ freq: number; confidence: number; note: string } | null>(null);
  ...
  useEffect(() => {
    ...
    return () => { try { engineRef.current?.ctx?.close(); } catch {} };
  }, []);
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        ...
        {audioBuffer && (
          <>
            <KpiGrid
              tiles={[
                { label: "Duration", value: `${audioBuffer.duration.toFixed(1)}s`, icon: <Play className="w-4 h-4" />, accent: "primary" },
                { label: "Sample Rate", value: `${(audioBuffer.sampleRate / 1000).toFixed(1)}kHz`, icon: <Gauge className="w-4 h-4" />, accent: "neutral" },
                { label: "Channels", value: audioBuffer.numberOfChannels, icon: <Radio className="w-4 h-4" />, accent: "neutral" },
                { label: "Samples", value: audioBuffer.length.toLocaleString(), icon: <Waves className="w-4 h-4" />, accent: "neutral" },
              ]}
            />
              ...
              <ProCard title="BPM Detection" icon={<Gauge className="w-4 h-4" />}>
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    <Badge variant="outline">Confidence: {(bpm.confidence * 100).toFixed(0)}%</Badge>
                    <p className="text-xs text-muted-foreground">{bpm.beats.length} beats detected</p>
                  </div>
                </div>
                <MeterBar value={bpm.confidence * 100} label="Confidence" color="bull" showValue className="mt-3" />
              </ProCard>
            )}
            ...
            {key && (
              <ProCard title="Key Detection" icon={<Music className="w-4 h-4" />}>
                <div className="flex items-center gap-4">
                  <span className="text-4xl font-bold text-primary">{key.key} {key.scaleType}</span>
            ...
            {lufs && (
              <ProCard title="LUFS Meter" icon={<Volume2 className="w-4 h-4" />}>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <StatTile label="Momentary" value={`${lufs.momentary.toFixed(1)} LUFS`} accent="neutral" />
                  <StatTile label="True Peak" value={`${lufs.truePeak.toFixed(1)} dB`} accent={lufs.truePeak > 0 ? "bear" : "bull"} />
                </div>
                <MeterBar
            ...
            {spectrum && (
              <ProCard title="Spectrum Analysis" icon={<Activity className="w-4 h-4" />}>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <StatTile label="Centroid" value={`${spectrum.centroid.toFixed(0)} Hz`} accent="primary" />
                  <StatTile label="Spread" value={`${spectrum.spread.toFixed(0)} Hz`} accent="neutral" />
                  <StatTile label="Flatness" value={spectrum.flatness.toFixed(3)} accent="neutral" />
                  <StatTile label="Rolloff" value={`${spectrum.rolloff.toFixed(0)} Hz`} accent="neutral" />
                </div>
              </ProCard>
            )}
              ...
              <ProCard title="Pitch Detection" icon={<Mic className="w-4 h-4" />}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Confidence: {(pitch.confidence * 100).toFixed(0)}%</p>
                  </div>
                </div>
              </ProCard>
            )}
            ...
            {fingerprint && (
              <ProCard title="Audio Fingerprint" icon={<Hash className="w-4 h-4" />}>
                <p className="text-2xl font-mono font-bold text-primary tracking-wider">{fingerprint}</p>
                <p className="text-xs text-muted-foreground mt-2">Unique identifier for this audio file. Use for matching and identification.</p>
              </ProCard>
            )}
          </>
        )}
