...
import { runMonteCarlo, quickRiskAssessment, type MonteCarloResult } from "@/lib/engine/monte-carlo";
...
export const Route = createFileRoute("/monte-carlo")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Run Monte Carlo simulations to stress-test your strategy and estimate probability of ruin." },
    ],
  }),
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Stress-test your strategy with {numSims.toLocaleString()} randomized simulations. Understand the range of possible outcomes.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div>
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground">Initial Balance ($)</label>
              ...
              <div>
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground">Average Risk:Reward</label>
                <input type="number" step="0.1" value={avgRR} onChange={e => setAvgRR(Number(e.target.value))}
                  className="w-full bg-input border border-border rounded px-2 py-1.5 text-xs font-mono mt-0.5" />
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground">Simulations</label>
              ...
              <div>
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground">Trades to Simulate</label>
                <input type="number" value={numTrades} onChange={e => setNumTrades(Number(e.target.value))}
                  className="w-full bg-input border border-border rounded px-2 py-1.5 text-xs font-mono mt-0.5" />
              </div>
              ...
              <div className="pt-3 border-t border-border space-y-2">
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Quick Assessment</div>
                <div className="flex justify-between text-[11px]">
                  <span className="text-muted-foreground">Expectancy</span>
                  <span className={`font-mono font-semibold ${quickAssess.expectancy > 0 ? "text-bull" : "text-bear"}`}>
                    {quickAssess.expectancy.toFixed(3)}R
                  </span>
                </div>
                <div className="flex justify-between text-[11px]">
                  <span className="text-muted-foreground">Kelly %</span>
                  <span className="font-mono font-semibold">{quickAssess.kellyPct.toFixed(1)}%</span>
                </div>
                <div className="flex justify-between text-[11px]">
                  <span className="text-muted-foreground">Ruin Estimate</span>
                  <span className={`font-mono font-semibold ${quickAssess.ruinEstimate > 0.1 ? "text-bear" : "text-bull"}`}>
            ...
            {!result ? (
              <Card>
                <CardContent className="py-16 text-center text-muted-foreground">
                  ...
                  <p className="text-xs mt-1">Results will show probability distributions and risk metrics</p>
                </CardContent>
              </Card>
            ) : (
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  <div className="p-3 rounded-lg glass-card text-center">
                    ...
                    <div className={`text-xl font-bold font-mono ${result.statistics.profitablePct > 60 ? "text-bull" : "text-bear"}`}>
                      {result.statistics.profitablePct.toFixed(0)}%
                    </div>
                  </div>
                  <div className="p-3 rounded-lg glass-card text-center">
                    <div className="text-[9px] text-muted-foreground uppercase">Ruin Prob</div>
                    <div className={`text-xl font-bold font-mono ${result.ruinProbability > 0.05 ? "text-bear" : "text-bull"}`}>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div className="space-y-2">
                      {[
                        { label: "Best Case (P95)", value: result.percentiles.p95, color: "bg-bull" },
                        { label: "Optimistic (P75)", value: result.percentiles.p75, color: "bg-bull/60" },
                        { label: "Median (P50)", value: result.percentiles.p50, color: "bg-primary" },
                        { label: "Conservative (P25)", value: result.percentiles.p25, color: "bg-medium" },
                        { label: "Worst Case (P5)", value: result.percentiles.p5, color: "bg-bear" },
                      ].map(p => (
                        <div key={p.label} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 h-4 bg-muted/30 rounded overflow-hidden">
                            <div
                              ...
                              style={{ width: `${Math.min(100, Math.max(0, (p.value / result.statistics.bestCase) * 100))}%` }}
                            />
                          </div>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {result.recommendations.map((rec, i) => (
                        <div key={i} className="text-[11px] text-muted-foreground leading-relaxed p-2 rounded bg-muted/20">
                          {rec}
                        </div>
                      ))}
                      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border">
                        <div className="p-2 rounded bg-primary/10 text-center">
                          <div className="text-[9px] text-muted-foreground uppercase">Optimal Kelly</div>
                          <div className="text-sm font-mono font-bold text-primary">{(result.optimalKelly * 100).toFixed(2)}%</div>
                        </div>
                        <div className="p-2 rounded bg-bull/10 text-center">
                          <div className="text-[9px] text-muted-foreground uppercase">Half Kelly (Recommended)</div>
                          <div className="text-sm font-mono font-bold text-bull">{(result.halfKelly * 100).toFixed(2)}%</div>
                        </div>
                      </div>
                    </div>
                  </CardContent>
                ...
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm uppercase tracking-wider">Detailed Statistics</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-[11px]">
                      {[
                        ...
                        ["Recovery Factor", result.statistics.meanMaxDrawdown > 0 ? (result.statistics.meanFinalEquity / result.statistics.meanMaxDrawdown).toFixed(2) : "N/A"],
