...
import { LineChart, Line, ResponsiveContainer, XAxis, YAxis, Tooltip, ReferenceLine, ComposedChart, Scatter, CartesianGrid, BarChart, Bar, Legend, Area } from "recharts";
import { History, Loader2, TrendingUp, TrendingDown, Download, Image as ImgIcon, Play, Pause, BarChart3, Target, Award, AlertTriangle, CheckCircle2, Clock, Settings, ChevronRight, Zap } from "lucide-react";
...
export const Route = createFileRoute("/backtest")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Professional walk-forward backtesting with equity curves, R-multiple analysis, strategy comparison, and auto-execution." },
    ],
  }),
...
function BacktestPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      toast.success(`Backtest complete: ${r.signals.length} signals, ${r.winRate.toFixed(1)}% win rate`);
    } catch (e: any) { setErr(e.message ?? String(e)); toast.error("Backtest failed: " + (e.message || String(e))); }
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const tick = async () => { if (!cancelled && !busy) await run(); };
    tick();
  ...
  const sharpe = useMemo(() => {
    ...
    const returns = result.equityCurve.slice(1).map((p, i) => p.equity - result.equityCurve[i].equity);
  ...
  const profitFactor = useMemo(() => {
    ...
    const losses = Math.abs(result.signals.filter(s => s.rMultiple < 0).reduce((a, b) => a + b.rMultiple, 0));
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Walk-forward simulation with equity analysis, R-multiples, and performance metrics</p>
          </div>
          <div className="flex gap-2">
            <Button variant={autoBacktest ? "default" : "outline"} onClick={() => setAutoBacktest(!autoBacktest)} size="sm" className={autoBacktest ? "bg-bull text-primary-foreground hover:bg-bull/90" : ""}>
              {autoBacktest ? <Pause className="w-4 h-4 mr-1.5" /> : <Play className="w-4 h-4 mr-1.5" />}
            ...
            <Button onClick={run} disabled={busy} size="sm" className="gap-2">
              {busy ? <><Loader2 className="w-4 h-4 animate-spin" /> Running...</> : <><Zap className="w-4 h-4" /> Run Backtest</>}
            </Button>
          </div>
        </div>
        ...
        {autoBacktest && (
          <Card className="border-bull/30">
            <CardContent className="p-3 flex items-center gap-4">
              ...
              <select value={autoInterval} onChange={e => setAutoInterval(+e.target.value)} className="bg-input border border-border rounded px-2 py-1 text-sm font-mono">
              ...
              {lastAutoRun && <span className="text-xs text-muted-foreground font-mono">Last: {lastAutoRun.toLocaleTimeString()}</span>}
            </CardContent>
          </Card>
        )}
        ...
        {showSettings && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-semibold uppercase tracking-wider">Test Parameters</CardTitle>
            </CardHeader>
            <CardContent className="pt-0">
              <div className="grid md:grid-cols-7 gap-3 items-end">
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono mb-1">Pair</div>
                ...
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono mb-1">Timeframe</div>
                ...
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono mb-1">Candles</div>
                ...
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono mb-1">Min Score</div>
                ...
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono mb-1">Cooldown</div>
                ...
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono mb-1">Forward Bars</div>
                ...
                <Button onClick={run} disabled={busy} className="h-9">{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : "Run"}</Button>
              </div>

              <div className="grid md:grid-cols-3 gap-3 mt-3">
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono mb-1">Spread (pips)</div>
                  <input type="number" min={0} step={0.1} value={settings.spreadPips} onChange={e => update({ spreadPips: +e.target.value })} className="w-full bg-input border border-border rounded px-2 py-1.5 text-sm" />
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono mb-1">Slippage (pips)</div>
                  <input type="number" min={0} step={0.1} value={settings.slippagePips} onChange={e => update({ slippagePips: +e.target.value })} className="w-full bg-input border border-border rounded px-2 py-1.5 text-sm" />
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-mono mb-1">Exec Delay (bars)</div>
                  <input type="number" min={0} step={1} value={settings.execDelayBars} onChange={e => update({ execDelayBars: +e.target.value })} className="w-full bg-input border border-border rounded px-2 py-1.5 text-sm" />
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        {result && (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              ...
              <MetricCard icon={TrendingUp} label="Wins" value={String(result.wins)} color="text-bull" />
              <MetricCard icon={TrendingDown} label="Losses" value={String(result.losses)} color="text-bear" />
              <MetricCard icon={Target} label="Win Rate" value={`${result.winRate.toFixed(1)}%`} color={result.winRate >= 50 ? "text-bull" : "text-bear"} />
              <MetricCard icon={Award} label="Total R" value={result.totalR.toFixed(2)} color={result.totalR >= 0 ? "text-bull" : "text-bear"} />
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <MetricCard icon={CheckCircle2} label="Avg R" value={avgR} />
              <MetricCard icon={AlertTriangle} label="Max Drawdown" value={maxDrawdown} color="text-bear" />
                ...
                <div style={{ height: 300 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={result.equityCurve}>
                      ...
                      <Area type="monotone" dataKey="equity" stroke="var(--elite)" fill="var(--elite)" fillOpacity={0.1} dot={false} strokeWidth={2} />
              ...
              {Object.entries(result.byRating).map(([r, v]) => (
                <Card key={r}>
                  <CardContent className="p-3">
                    ...
                    <div className="text-lg font-bold font-mono">{v.count} <span className="text-xs text-muted-foreground">trades</span></div>
                    ...
                    <div className="h-1.5 bg-muted rounded-full mt-2 overflow-hidden">
                      <div className={`h-full ${v.winRate >= 50 ? "bg-bull" : "bg-bear"}`} style={{ width: `${v.winRate}%` }} />
                    </div>
                  </CardContent>
                </Card>
              ))}
        ...
        {!result && !err && !busy && (
          <Card className="border-dashed">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">Configure test parameters and click Run Backtest to begin analysis.</p>
            </CardContent>
          </Card>
        )}
      </div>
