...
export class SocialSentimentTracker {
  ...
  private symbolHistory: Map<string, { timestamp: number; sentiment: number; volume: number }[]> = new Map();
  ...
  getTrend(symbol: string, windowMs = 3600000): SentimentTrend {
    ...
    const previous = history.filter((h) => now - h.timestamp >= windowMs && now - h.timestamp < windowMs * 2);

    const currentSentiment = recent.length > 0
      ? recent.reduce((sum, h) => sum + h.sentiment, 0) / recent.length
      : 0;
    const previousSentiment = previous.length > 0
      ? previous.reduce((sum, h) => sum + h.sentiment, 0) / previous.length
      : 0;
    ...
    const influencerPosts = this.posts.filter((p) => p.mentions.includes(symbol) && p.authorFollowers > this.influencerThreshold);
    const influencerSentiment = influencerPosts.length > 0
  ...
  private calculateViralScore(symbol: string): number {
    const recent = this.posts.filter((p) => p.mentions.includes(symbol) && Date.now() - p.timestamp < 3600000);
    if (recent.length === 0) return 0;
    const totalEngagement = recent.reduce((sum, p) => sum + p.engagement.likes + p.engagement.retweets + p.engagement.replies + p.engagement.shares, 0);
    const avgEngagement = totalEngagement / recent.length;
    return Math.min(100, Math.log10(avgEngagement + 1) * 20);
  }

  private getPlatformBreakdown(symbol: string): { platform: string; sentiment: number; volume: number }[] {
  ...
  private checkAlerts(symbol: string, sentiment: number, change: number, velocity: number, viralScore: number): void {
  ...
  getTopInfluencers(symbol: string, limit = 10): { author: string; followers: number; avgSentiment: number; postCount: number }[] {
    const influencers = this.posts
      .filter((p) => p.mentions.includes(symbol) && p.authorFollowers > this.influencerThreshold)
      .reduce((acc, post) => {
        if (!acc[post.author]) acc[post.author] = { author: post.author, followers: post.authorFollowers, totalSentiment: 0, postCount: 0 };
        acc[post.author].totalSentiment += post.sentiment;
        acc[post.author].postCount++;
        return acc;
      }, {} as Record<string, { author: string; followers: number; totalSentiment: number; postCount: number }>);

    return Object.values(influencers)
      .map((i) => ({ author: i.author, followers: i.followers, avgSentiment: i.totalSentiment / i.postCount, postCount: i.postCount }))
