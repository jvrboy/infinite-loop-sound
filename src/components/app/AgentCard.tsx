...
const statusConfig = {
  ...
  disabled: { icon: <Clock className="w-3 h-3" />, color: "text-muted-foreground", bg: "bg-muted/10", border: "border-border/50", label: "Disabled" },
};

export function AgentCard({ name, description, status, accuracy, lastRun, version, icon, onClick, className }: AgentCardProps) {
    ...
    >
      <CardHeader className="pb-2">
        <div className="flex items-start justify-between">
          ...
          <Badge variant="outline" className={cn("text-[9px] gap-1", cfg.color, cfg.bg, cfg.border)}>
            {cfg.icon}
            {cfg.label}
          </Badge>
        </div>
      ...
      <CardContent className="pt-0">
        ...
        <div className="flex items-center justify-between text-[10px] font-mono">
          {accuracy !== undefined && (
            <span className={cn("flex items-center gap-1", accuracy >= 70 ? "text-bull" : accuracy >= 50 ? "text-amber-400" : "text-bear")}>
