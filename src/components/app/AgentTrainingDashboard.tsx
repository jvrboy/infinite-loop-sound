...
import { Brain, Zap, Target, TrendingUp, Activity, Cpu, Play, Square, RefreshCw } from "lucide-react";
...
const AGENTS: Omit<AgentState, "accuracy" | "trained" | "trainingProgress" | "lastTrained" | "runs" | "wins">[] = [
...
export function AgentTrainingDashboard() {
  ...
  const trainAgent = useCallback(async (id: string) => {
    setTraining(id);
    ...
    for (let pct = 0; pct <= 100; pct += 5) {
      ...
      const updated = agents.map((a) =>
        a.id === id ? { ...a, trainingProgress: pct, accuracy: Math.min(99, a.accuracy + Math.random() * 2) } : a,
      );
      setAgents(updated);
    }
  ...
  const avgAccuracy = agents.length > 0
    ? Math.round(agents.reduce((a, b) => a + b.accuracy, 0) / agents.length)
    : 0;
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        action={
          <Button onClick={trainAll} disabled={globalRunning} className="gap-2">
            {globalRunning ? <><RefreshCw className="w-4 h-4 animate-spin" /> Training all...</> : <><Play className="w-4 h-4" /> Train All Agents</>}
          </Button>
        }
      />
      ...
      <KpiGrid
        tiles={[
          { label: "Active Agents", value: agents.filter((a) => a.enabled).length, sub: `of ${agents.length} total`, icon: <Cpu className="w-4 h-4" />, accent: "primary" },
          { label: "Trained", value: trainedCount, sub: `${Math.round((trainedCount / agents.length) * 100)}% coverage`, icon: <Target className="w-4 h-4" />, accent: "bull" },
          { label: "Avg Accuracy", value: `${avgAccuracy}%`, sub: "across all agents", icon: <TrendingUp className="w-4 h-4" />, accent: "bull" },
          { label: "Win Rate", value: `${winRate}%`, sub: `${totalWins}W / ${totalRuns}R`, icon: <Activity className="w-4 h-4" />, accent: winRate >= 60 ? "bull" : "warning" },
        ]}
      />
        ...
        {agents.map((agent) => (
          ...
          >
            <div className="space-y-3">
              ...
              <MeterBar value={agent.accuracy} color={agent.accuracy >= 70 ? "bull" : agent.accuracy >= 50 ? "warning" : "bear"} />
              ...
              <div className="flex gap-2 pt-1">
                ...
                >
                  {training === agent.id ? <RefreshCw className="w-3 h-3 animate-spin" /> : <Play className="w-3 h-3" />}
                  Train
                </Button>
                <Button
                  size="sm"
                  ...
                  className="gap-1.5"
                >
                  {agent.enabled ? <><Square className="w-3 h-3" /> Disable</> : <><Play className="w-3 h-3" /> Enable</>}
                </Button>
              </div>
            </div>
          </ProCard>
