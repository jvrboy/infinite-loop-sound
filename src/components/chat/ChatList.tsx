...
export function ChatList({
  threads, folders, activeId, onPick, onRename, onDelete, onTogglePin, onToggleArchive,
  onClone, onRestore, onMove, showArchived, showTrash,
  ...
  const renderRow = (t: Thread) => {
    ...
    return (
      <div key={t.id} className={`group relative flex items-center gap-2 p-2 rounded transition-colors ${isActive ? "bg-muted" : "hover:bg-muted/40"}`}>
        <button type="button" onClick={() => onPick(t.id)} className="flex items-center gap-2 flex-1 min-w-0 text-left">
          {t.pinned ? <Pin className="w-3 h-3 text-amber-400 shrink-0" /> : <MessageSquare className="w-3.5 h-3.5 text-muted-foreground shrink-0" />}
          {isEditing ? (
            <input autoFocus value={draft} onChange={(e) => setDraft(e.target.value)}
              onBlur={() => { if (draft.trim()) onRename(t.id, draft.trim()); setEditing(null); }}
              onKeyDown={(e) => { if (e.key === "Enter") { if (draft.trim()) onRename(t.id, draft.trim()); setEditing(null); } if (e.key === "Escape") setEditing(null); }}
              className="bg-background border border-border rounded px-1.5 py-0.5 text-xs flex-1 min-w-0" />
          ) : (
            <span className="text-xs truncate">
              {t.title}
              {t.clonedFrom && <span className="text-[9px] text-muted-foreground ml-1">(clone)</span>}
            </span>
          )}
        </button>
        ...
        {openMenu === t.id && (
          <div onMouseLeave={() => setOpenMenu(null)} className="absolute right-2 top-full mt-1 z-20 rounded-md border border-border bg-popover shadow-md p-1 w-48">
            <button className="flex items-center gap-2 w-full text-left px-2 py-1.5 text-xs rounded hover:bg-muted"
              onClick={() => { setEditing(t.id); setDraft(t.title); setOpenMenu(null); }}>
              <Pencil className="w-3 h-3" /> Rename
            </button>
            <button className="flex items-center gap-2 w-full text-left px-2 py-1.5 text-xs rounded hover:bg-muted"
              onClick={() => { onTogglePin(t.id); setOpenMenu(null); }}>
              {t.pinned ? <PinOff className="w-3 h-3" /> : <Pin className="w-3 h-3" />}
              {t.pinned ? "Unpin" : "Pin"}
            </button>
            <button className="flex items-center gap-2 w-full text-left px-2 py-1.5 text-xs rounded hover:bg-muted"
              onClick={() => { onToggleArchive(t.id); setOpenMenu(null); }}>
              {t.archived ? <ArchiveRestore className="w-3 h-3" /> : <Archive className="w-3 h-3" />}
              {t.archived ? "Unarchive" : "Archive"}
            </button>
            <button className="flex items-center gap-2 w-full text-left px-2 py-1.5 text-xs rounded hover:bg-muted"
              onClick={() => { onClone(t.id); setOpenMenu(null); }}>
              <Copy className="w-3 h-3" /> Clone
            </button>
            {showTrash && (
              <button className="flex items-center gap-2 w-full text-left px-2 py-1.5 text-xs rounded hover:bg-muted text-emerald-400"
                onClick={() => { onRestore(t.id); setOpenMenu(null); }}>
            ...
            <button className="flex items-center gap-2 w-full text-left px-2 py-1.5 text-xs rounded hover:bg-muted"
              onClick={() => { onMove(t.id, null); setOpenMenu(null); }}>
              <MessageSquare className="w-3 h-3" /> No folder
            </button>
            {folders.map((f) => (
              <button key={f.id} className="flex items-center gap-2 w-full text-left px-2 py-1.5 text-xs rounded hover:bg-muted"
                onClick={() => { onMove(t.id, f.id); setOpenMenu(null); }}>
      ...
      <div key={f.id} className="space-y-0.5">
        <div className="group flex items-center gap-1 px-2 py-1">
          <button onClick={() => toggleFolder(f.id)} className="flex items-center gap-1 flex-1 min-w-0">
            ...
            {folderEditing === f.id ? (
              <input autoFocus value={folderDraft} onChange={(e) => setFolderDraft(e.target.value)}
                onBlur={() => { if (folderDraft.trim()) onRenameFolder(f.id, folderDraft.trim()); setFolderEditing(null); }}
                onKeyDown={(e) => { if (e.key === "Enter") { if (folderDraft.trim()) onRenameFolder(f.id, folderDraft.trim()); setFolderEditing(null); } if (e.key === "Escape") setFolderEditing(null); }}
                className="bg-background border border-border rounded px-1.5 py-0.5 text-xs flex-1 min-w-0" />
            ) : (
          ...
          <Button size="icon" variant="ghost" className="h-5 w-5 opacity-0 group-hover:opacity-100"
            onClick={(e) => { e.stopPropagation(); setFolderEditing(f.id); setFolderDraft(f.name); }}>
  ...
  if (visible.length === 0 && folders.length === 0) {
    return (
      <p className="text-xs text-muted-foreground italic px-2 py-4">
        {showTrash ? "Trash is empty." : showArchived ? "No archived chats." : "No chats yet. Start one!"}
      </p>
    );
  }
  ...
  return (
    <div className="space-y-1">
      ...
      {unfoldered.length > 0 && folders.length > 0 && (
        <div className="px-2 py-1 text-[10px] uppercase tracking-wider text-muted-foreground">No folder</div>
      )}
